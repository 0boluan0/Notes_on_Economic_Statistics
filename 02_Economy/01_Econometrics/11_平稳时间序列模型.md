...
#  **分布滞后模型Distributed Lag Model [[Distributed Lag Model|DLM]]**

## 含义

分布滞后模型是静态模型，但它反映了解释变量的动态影~~像~~响

[[Distributed Lag Model|分布滞后模型]]（[[Distributed Lag Model|Distributed Lag Model]], [[Distributed Lag Model|DLM]]）是一种时间序列模型，用于描述当前因变量不仅受当前解释变量的影响，还受其过去若干期的解释变量的影响。分布滞后模型广泛用于经济学、金融学等领域，分析变量之间的动态关系。
...
4. **滞后分布**：滞后系数 $\beta_0, \beta_1, \beta_2, \ldots, \beta_q$ 描述了解释变量对因变量的动态影响。系数和需收敛（不能趋于无穷大），这也意味着解释变量对被解释变量的长期影响是有限的，且随时间推移减小。

## **分布滞后模型的估计**

分布滞后模型的参数通常通过最小二乘法（[[OLS Basics|OLS]]）估计。
...
* 根号项需为非负；~~不然无意义~~ 否则统计量未定义，h 检验无效。
* 如果包含多个解释变量和多个~~值哦呼~~滞后，被解释变量也是有效的。

#### **计算步骤**

1. **估计回归模型**：
使用普通最小二乘法（[[OLS Basics|OLS]]）估计回归模型，得到回归系数和残差。
...
通过 [[OLS Basics|OLS]] 回归结果得到 $\beta_2$ 的方差估计值。
...
参见：[[ARMA]] · [[自相关函数]] · [[偏自相关函数]] · [[平稳性]]

## 时间序列分析的几个基本概念

### 随机过程

随机变量在时间轴上的表现；每个时点都有其期望、方差与分布特征。~~啥的~~
...
### **自回归模型（Autoregressive Model, [[AR(1) stationarity|AR]]）的定义**
...
一阶自回归模型（[[AR(1) stationarity|AR]](1)模型）可以表示为：
...
由 [[AR(1) stationarity|AR]](p)模型平稳性可知， MA(1)模型具有可逆性的条件是$|\theta|$<1。更一般地， 任何一个可逆的MA(q)模型可转换成一个无限阶的自回归模型
...
[[ARMA|ARMA模型]]，即自回归移动平均模型（[[ARMA|Autoregressive Moving Average Model]]），是用于时间序列分析的常见模型。ARMA模型结合了自回归模型（[[AR(1) stationarity|AR]]）和移动平均模型（MA）的特性，用于捕捉时间序列数据中的动态特征。

自相关系数和偏自相关系数都是拖尾的.

### **[[ARMA|ARMA]](p, q)模型的形式**

[[ARMA|ARMA]](p, q)模型可以表示为：
...
VAR模型由多个自回归方程组成，每个方程描述一个时间序列变量与所有变量的滞后值之间的关系。假设我们有 $k$ 个时间序列变量，每个变量在时间 $t$ 的值分别为 $Y_{1,t}, Y_{2,t}, \ldots, Y_{k,t}$，[[VAR Model|VAR]](p)模型可以表示为：
...
在矩阵记号下，脉冲响应函数可由 [[VMA|VMA]] 系数矩阵 $\Theta_h$ 给出：
$$
\mathbf y_t=\sum_{h=0}^{\infty}\Theta_h \boldsymbol\epsilon_{t-h},
$$
其中 $\Theta_h$ 的第 $(i,j)$ 元就是“对第 $j$ 个冲击在第 $h$ 期对第 $i$ 个变量的响应”。
