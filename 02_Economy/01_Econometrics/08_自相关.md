...
# [[Autocorrelation|自相关]]([[Autocorrelation|Autocorrelation]])的含义和表现形式
...
对误差项的相关性可抽象写为：
$$
E(\varepsilon_i\varepsilon_j)\neq 0,\qquad i,j\in T,\ i\neq j.
$$

常见的一阶自回归误差形式可写为
$$
\varepsilon_t=\rho\,\varepsilon_{t-1}+v_t,\qquad |\rho|<1.
$$
其中 $\rho$ 为自相关系数，$v_t$ 为白噪声。
...
首先，使用普通最小二乘法（[[OLS Basics|OLS]]）估计原始回归模型的参数，得到初始残差。  
...
首先，使用普通最小二乘法（[[OLS Basics|OLS]]）估计回归模型，得到初始的残差：
...
## [[Newey-West|Newey]]-west修正标准误

**Step 1: 回归模型的估计**

首先，我们考虑一个简单的线性回归模型：
$$ y_t = X_t \beta + \epsilon_t $$
使用普通最小二乘法（[[OLS Basics|OLS]]）来估计回归系数 $\beta$，即：
$$ \hat{\beta} = (X’X)^{-1}X’y $$
