# 概念与定义(Concepts and Definitions)
<!-- bilingual-en:start -->
*Concepts and Definitions*
<!-- bilingual-en:end -->

**一个人能用，别人也能用，我们称之为“没有竞争性”，一个人能用了别人就不能用，我们称之为有“竞争性**
**一件商品在实际使用时，雇主能不能够方便的把其他的使用者排除在外，如果能比较方便地做到，就说明它有“排他性”，不方便做到，就说明它不具有“排他性”。**
a 王 提 Fx v= eS IN WAS RR 非 竞 争 性 有 线 电视 、 收 费 软 件 | 无 线 广播 、 开 源 软
<!-- bilingual-en:start -->
**A good is non-rival when one person's use does not prevent another person's use. It is rival when one person's use leaves less, or none, for others.**
**A good is excludable when a provider can readily prevent non-payers from using it. It is non-excludable when doing so is difficult or impractical.**
The last line is corrupted OCR contrasting examples such as cable television and paid software with wireless broadcasting and open-source software.
<!-- bilingual-en:end -->
![[Pasted image 20240312133200.png]]
* 纯公共物品(Pure [[物品的竞争性与排他性|Public Goods]])：非竞争性，非排他性
* 私人物品(Private Goods)：竞争性，排他性
* [[物品的竞争性与排他性|俱乐部物品]](Club Goods)：非竞争性，排他性
* [[物品的竞争性与排他性|公共资源]](Common Resources)：竞争性，非排他性
<!-- bilingual-en:start -->
* Pure public goods are non-rival and non-excludable.
* Private goods are rival and excludable.
* Club goods are non-rival but excludable.
* Common-pool resources are rival but non-excludable.
<!-- bilingual-en:end -->

举例：
- Basic research（基础科研）
- Uncongested non-toll roads（非收费、非拥堵公路）
- Fireworks display（烟火秀）
- National defense（国防）
<!-- bilingual-en:start -->
Examples:
- Basic research
- Uncongested non-toll roads
- Fireworks displays
- National defence
<!-- bilingual-en:end -->

特别的，公共物品的分类不是绝对的，有时候会因情况的变动而改变。
— e.g., Lighthouse (灯塔 ) & signal receiver (nonexcludability criterion does not hold) —eg., SHLAA fd vs. 拥 墙 的 公园 (nonrivalness criterion is no longer satisfied) — Impure public good 非 纯 公 共 物 品 : to some extent rival or excludable en - 3}
<!-- bilingual-en:start -->
These categories are not absolute; a good can move between them when congestion, technology, or exclusion arrangements change.
The second line is corrupted OCR giving examples such as a lighthouse paired with a signal receiver, an uncongested versus congested park, and an impure public good that is rival or excludable to some degree.
<!-- bilingual-en:end -->
- 例如：灯塔与信号接收（nonexcludability 不完全成立）。
- 例如：不拥挤的公园 vs. 拥挤的公园（nonrivalness 条件不再完全满足）。
- **Impure public good（非纯公共物品）**：在一定程度上具有竞争性或可排他性。
<!-- bilingual-en:start -->
- For example, pairing a lighthouse with a signal receiver can make exclusion feasible, so non-excludability no longer fully holds.
- An uncongested park may be non-rival, while congestion makes additional use rival.
- **An impure public good** is rival or excludable to some degree.
<!-- bilingual-en:end -->

# 私人物品的有效提供
<!-- bilingual-en:start -->
*Efficient Provision of Private Goods*
<!-- bilingual-en:end -->
给每个人提供的数量不一样但是价格一样。
<!-- bilingual-en:start -->
Different consumers may purchase different quantities, but they face the same market price.
<!-- bilingual-en:end -->
$$ \left[\frac{\mathrm{Pree~}{\mid\vphantomobject Object{\enspace0.5~}}{\mid\cfrac{\mathrm{Gve}}{\mid\cfrac{\mathrm{Ate}}{\mid\mathrm{A}}}}}{\mid\cfrac{\mathfrak{I}}{\mid\cfrac{\mathfrak{A}}{\mid\cfrac{\mathrm{A}}{\mid\mathrm{A}}}}{\mid\cfrac{\mathfrak{A}}{\mid\cfrac{\mathrm{A}}{\mid\cfrac{\mathrm{A}}{\mid\mathrm{A}}}}{\mid\cfrac{\mathrm{A}}{\mid\Upsilon}}}}\right] $$
![[Pasted image 20240314092642.png]]
$$ \begin{array}{l}{{\frac{\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma}}{\left|\mp\Gamma\right|}}}\\ {{\frac{\Gamma\Gamma\left|\overline{{{\bf\Psi}}}}{\pm\Gamma\Lambda\overline{{{\bf\Psi}}}\Sigma\Gamma\Sigma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\overline{{{\bf\Psi}}}}{\left|\frac{\Gamma\Gamma\overline{{{\bf\Psi}}}}{\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\Gamma\overline{{{\bf\Psi}}}}\overline{{{{\bf\Psi}}}}\Gamma\overline{{{\bf\Psi}}}\overline{{{\bf\Psi}}}}{\left|\frac{\Gamma\overline{{{\bf\Psi}}}}}\overline{{{\bf\Psi}}}\right|}}}\\ {{\frac{\left|\overline{{{\bf\Psi}}}}\left|\Pi\overline{{\Pi}}\right|\Pi\overline{{\Pi\Pi}}\overline{{{\Pi}}}}\Pi\Pi\Sigma\Pi\Sigma\right|\Pi\Sigma\right|}}}}\\ {{{{\Sigma}}}}{{{\Pi}}}}}\end{array}{\right.\right. $$
![[Pasted image 20240314092652.png]]
每个人MRS相同，但个人可以消费不同的数量。总需求是对不同数量的横向加总。
<!-- bilingual-en:start -->
At the efficient allocation, consumers have the same MRS, although they may consume different quantities. Market demand is obtained by horizontally summing the quantities demanded at each price.
<!-- bilingual-en:end -->
# 公共物品的有效提供
<!-- bilingual-en:start -->
*Efficient Provision of Public Goods*
<!-- bilingual-en:end -->
ee ee Market $550 $450 $350 $250 (DA**) ed ttl
<!-- bilingual-en:start -->
The line is corrupted OCR from the willingness-to-pay diagram, with values of $550, $450, $350, and $250.
<!-- bilingual-en:end -->
![[Pasted image 20240314095014.png]]
每个人的支付意愿不同，但是提供的数量完全相同
<!-- bilingual-en:start -->
Individuals may have different willingness to pay, but everyone consumes the same supplied quantity of the public good.
<!-- bilingual-en:end -->
$$ \begin{array}{c}{{{}_{n}\frac{\mathrm{\bigg:}\!}{}_{n}\frac{\mathrm{\bigg|}--\mathrm{\boldmath~---\mathrm{\boldmath~--~}\mathrm{\bf--~}}-\mathrm{\boldmath~--~}\mathrm{\boldmath~--~}\mathrm{\boldmath~-~}}}\\ {{{}_{n}\mathrm{\scriptsize~\bar{\scriptstyle\leq}\!}\quad{}\!\!\!\!\!}}&{{{}\!\!\!\!}}\\ {{{}^{\quad}\!\!\!}_{n}^{\mathrm{\scriptsize~\ell}\!\!\!\leq\!\!\!\!--\mathrm{\boldmath~--\mathrm{\boldmath~--\mathrm{\boldmath~-~}\!\!~--\mathrm{\boldmath~-~}-\mathrm{\boldmath~-~}-\mathrm{\boldmath~-~}}-\mathrm{\boldmath~-~}\mathrm{\boldmath~-~}}}\\ {{{}\!\!\!\!\!}}&{{{}\,\,\,~~~~~~i}}\end{array} $$
![[Pasted image 20240314095046.png]]
每个人消费的数量相同，但人们可以有不同的MRS。通过纵向加总，得到集体支付意愿。
<!-- bilingual-en:start -->
Everyone consumes the same quantity, but individuals may have different MRS values. Vertically summing their willingness to pay gives the collective marginal willingness to pay.
<!-- bilingual-en:end -->

## 公共物品提供时的问题
<!-- bilingual-en:start -->
*Problems in Providing Public Goods*
<!-- bilingual-en:end -->

市场难以有效提供公共物品，
<!-- bilingual-en:start -->
Markets have difficulty providing public goods efficiently.
<!-- bilingual-en:end -->

* 搭便车：未付费者仍能受益时，个人有动机少贡献或低报支付意愿；私人决策没有计入给他人的全部边际收益，因此总供给可能低于有效量。
* 完全的价格歧视很难实现。
<!-- bilingual-en:start -->
* Free riding: when non-payers still benefit, individuals have an incentive to contribute less or understate willingness to pay. Private decisions omit some benefits to others, so total provision may fall below the efficient level.
* Perfect price discrimination is difficult to implement.
<!-- bilingual-en:end -->
