...
债券定义、股票的定义(不仅仅股票是什么还有什么样的分类)、比如1.3.3 不同的分类方法,要记住、可转换债券.（不知道是不是债券的分类也要了解清楚）(Bond definition, stock definition (not [[内生性、识别、IV、2SLS 与 GMM|just]] what stocks are but also different classification methods, e.g., 1.3.3 different classification methods, need to remember), convertible bonds. (Not sure if bond classification also needs to be clearly understood))
...
4. 债券筹资发行对象广，筹资额大(Bond financing has a [[平稳性、遍历性与谱|wide]] range of issuance targets and large financing amounts)
<!-- bilingual-en:start -->
...
## 2.4 债券收益率的计算(Calculation of Bond [[利率、即期远期与期限结构|Yield]])
<!-- bilingual-en:start -->
...
若给定某种利率或利息模型，$a(t)$ 反映了资金随时间增长的倍数。比如简单复利时，$a(t) = (1 + i)^t$；若为连续复利，$a(t) = e^{\delta t}$ 等等。(Given an interest rate or interest model, $a(t)$ reflects the growth multiple of funds [[内生性、识别、IV、2SLS 与 GMM|over]] time. For example, simple compound interest: $a(t) = (1 + i)^t$; continuous compound interest: $a(t) = e^{\delta t}$, etc.)
<!-- bilingual-en:start -->
...
$$a^{-1}(t) = \frac{1}{a(t)}$$
称为“贴现函数”或“折现函数”，它表示在时刻 $0$ 看来，$1$ 元在时刻 $t$ 的现值有多少。(Called "discount function", it represents the [[货币时间价值与贴现|present value]] of 1 yuan at time $t$ as viewed from time $0$)
<!-- bilingual-en:start -->
The reciprocal
$$a^{-1}(t) = \frac{1}{a(t)}$$
...
#### 2.4.1.3利率和贴现率(Interest Rate and [[货币时间价值与贴现|Discount Rate]])
<!-- bilingual-en:start -->
...
==期初计息：[[货币时间价值与贴现|贴现率]]==(==Beginning-of-period interest calculation: [[货币时间价值与贴现|discount rate]]==)
<!-- bilingual-en:start -->
...
>==觉得别扭的主要是因为贴现率的概念在日常场景不太常用。你只要记住：**贴现率 = 利息 ÷ 期末总额**，它测的是“利息在期末总额中所占的份额”。==(==The discomfort is mainly because the concept of discount rate is not commonly used in daily scenarios. [[内生性、识别、IV、2SLS 与 GMM|Just]] remember: **discount rate = interest ÷ final amount**, it measures the "proportion of interest in the final amount"==)
><!-- bilingual-en:start -->
...
基本年金 :等时间间隔付款,付款频率与利息转换频率一致,每次付款金额恒定(Basic [[货币时间价值与贴现|annuity]]: payments at equal time intervals, payment frequency consistent with interest conversion frequency, constant payment amount each time)
...
<!-- bilingual-en:end -->
##### **(1). 延付年金现值**(**(1) [[货币时间价值与贴现|Present Value]] of Ordinary Annuity**)
<!-- bilingual-en:start -->
...
##### **(3). 后付年金终值(累积值)**(**(3) [[货币时间价值与贴现|Future Value]] of Ordinary Annuity (Accumulated Value)**)
<!-- bilingual-en:start -->
...
汇率风险:各种货币组合
[[银行账簿利率风险：重定价缺口与 Duration Gap|利率风险]]:利率的反向指标--债券价格
<!-- bilingual-en:start -->
Exchange Rate Risk: Various Currency Combinations
[[银行账簿利率风险：重定价缺口与 Duration Gap|Interest Rate Risk]]: An inverse indicator of interest rates -- bond prices
<!-- bilingual-en:end -->
