...
衍生产品交易的信用风险比传统贷款的信用风险更加复杂。核心特点包括：

## 1.1 动态风险敞口

贷款的风险敞口在发放时就确定（如5年期1000万美元贷款，风险敞口基本保持1000万不变），而衍生产品的未来风险暴露（违约时可能遭受的损失）是不确定的，会随市场变化而波动。

**示例**：银行与客户做5年期利率互换：
- 若互换对银行现值为正（客户欠银行钱），风险敞口等于互换合约当前价值
- 若互换对银行价值为负（银行欠客户），风险敞口为零

**结论**：衍生品交易的敞口动态变化，只有当交易对手的合约市值对我们为正时才存在信用暴露风险。

## 1.2 信用风险缓释机制

为降低衍生品交易的信用风险，场外衍生品通常采用以下机制：

### 双边净额清算

国际掉期与衍生工具协会（ISDA）的主协议允许对同一对手的多笔交易进行净额结算：
- 在一方违约或提前终止事件时，双方所有未了结交易归并为一笔交易进行净结算
- 显著降低信用风险：将多笔交易的敞口由逐笔相加的大值降低为净敞口

### 抵押品（保证金）

要求交易对手提供抵押品以覆盖潜在风险敞口。

### 主协议补充条款

如**降级触发条款**：当交易一方的信用评级被下调到某一门槛以下时，该方须向对手方提供额外的抵押品。

# 2. CVA与DVA的定义、公式及意义

## 2.1 CVA（信用价值调整）

**信用价值调整**（CVA, Credit Valuation Adjustment）是指衍生产品交易商对因交易对手可能违约所导致损失的**预期损失值**的估计。

### 核心概念
- CVA反映了交易对手信用风险对衍生品价值的影响
- 在计价时，应从假定无违约情况下的衍生品风险中性价值中**减去CVA**
- 会计公式：衍生品的公允价值 = 假设无违约的价格 - CVA + DVA
- CVA总和的变动需要在当期损益中体现
