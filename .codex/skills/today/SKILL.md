...
- Treat Listening and Reading as input routes into one shared Writing tree. Using the plan's dynamic task pool, normally schedule at most one highest-value guided LN905 part per day in addition to `词灵`; never pair both modalities by habit. Use an input-route part only when the latest evidence shows that the meaning/evidence map itself is unreliable. When the map is usable, choose the weakest shared Writing node instead. If one shared Writing node remains weak in two comparable independent outputs, prioritize that node in the next two guided parts.
...
- Every scheduled guided LN905 part has one linked Markdown record containing exactly one part-specific init prompt and one `student-os:learning-log` block; its prompt names only that canonical task, requires a dedicated thread, begins with the five-field orientation card, states a concrete output and integration action, prohibits recall-dependent or copy-back exercises and stops at that part's completion.
- No ordinary day has more than one guided LN905 part scheduled unless the plan records a specific evidence-based exception; `词灵` does not count toward this limit. A Friday sample used for calibration remains byte-for-byte unchanged in the course Inbox, and any missing or limited calibration source is stated explicitly.
