...
3. For Listening, select one previously unused video from the verified official TED YouTube channel, preferably 15–20 minutes, and verify the matching TED transcript. Create the HTML at the task's fixed path with the support level required by the canonical task, timer when useful, autosave, word count, structured working fields, one agreed training focus, and post-completion transcript access. Do not turn ordinary teaching practice into an unsupported full attempt. If the session needs suggested topic vocabulary, record it in the artifact without creating another task or calendar block.
4. For Reading, use the number and type of sources required by the canonical task: one source for narrow comprehension/summary work, two for explicit synthesis practice, and three for a full simulation unless the task states otherwise. Use previously unused peer-reviewed papers that genuinely support the task's comparison or debate, and create accurate exam-style adapted extracts rather than copying long passages. At the task's fixed basename, preserve the `.tex`, compile the printable `.pdf`, and create a standalone response-and-review `.html`. The PDF contains only the question, required extracts and source attributions; the HTML contains the task-appropriate teaching support, source/evidence fields, response area, autosave, criteria checks and durable AI-review record. Render every PDF page and inspect it before marking the session ready.
...
9. In guided practice, use the teaching and feedback checkpoints specified by the canonical task. When a task asks for a small independent attempt, close the loop in the same conversation: AI diagnoses and asks at most three genuinely diagnostic questions; the user answers and revises; AI explicitly confirms whether to continue. After a draft, AI gives criteria-linked feedback and one high-value revision target, then verifies the returned revision. Both Listening and Reading HTML must copy the current autosaved fields into each checkpoint payload and support resending revised fields.
10. Give every guided Listening or Reading HTML an append-only block between `student-os:ai-review:start/end`, and make each copied checkpoint prompt name the fixed HTML path as `RECORD TARGET`. On every checkpoint turn, append a timestamped user-field snapshot, AI teaching/diagnosis/questions, user answer or revision, and gate decision to that block before replying. Preserve earlier entries and the existing localStorage key/fields; never remove the log when reusing a ready artifact, and do not append an exact duplicate of the latest checkpoint snapshot. The final payload includes all relevant notes, evidence/structure fields, response, revision and reflection. Treat localStorage as a working draft and the HTML block as the canonical review archive. If the write fails, report that the record was not saved.
