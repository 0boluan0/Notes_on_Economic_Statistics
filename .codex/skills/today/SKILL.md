...
- Show the exact word, phrase, collocation or frame on the front. Ask only for its Chinese meaning/function and ordinary use before showing the back. Never require a self-made sentence, essay scenario or skills diagnosis; observe actual production in end-to-end writing. Keep each item brief; total time follows the eligible items actually due. After each attempt, append the result to that card and update `last_recall`, `last_result`, `recall_stage` and `next_recall`.
