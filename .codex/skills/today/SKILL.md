...
A manual shutdown request may run the same factual-summary and reconciliation flow early. After that reconciliation, run the embedded shutdown recall below. There is no fixed evening time and no “end today” button requirement.

## Embedded LN905 recall

- Treat `99_学习情况记录/teach/LN905 Skill Bank/Cards/` as the atomic-card store linked from `LN905 Skill Bank.md`. Daily recall uses only the same production-language subset as Anki: exam-use words, phrases, collocations, sentence/discussion/cohesion frames, compact grammar/editing/style rules, and pronunciation/delivery targets. Upsert an existing card instead of creating a synonym duplicate.
- Exclude task analysis, source mapping, evidence selection, paragraph planning, note-taking, reading/listening strategies and other procedural capability cards from daily recall. Teach and observe them only inside end-to-end practice or timed calibration; an old `next_recall` date on an ineligible card does not make it surface.
- When an eligible language item is first taught or actually used, set its first recall for that day's shutdown. After a correct recall, advance by `+1, +3, +7, +14, +30` days; after a partial or incorrect recall, show it the next day; no answer leaves the due date unchanged and is not failure.
- At daily opening, place every eligible language card with `next_recall` on or before today in the single `student-os:opening-recall:start/end` block. At manual shutdown, place every eligible card still due or overdue plus every eligible card newly learned or used since opening whose first recall is due that day in the single `student-os:shutdown-recall:start/end` block. Never sample or cap. A card answered and rescheduled at opening is not repeated at shutdown unless new evidence explicitly makes another same-day recall due. If automatic next-morning reconciliation cannot obtain an answer, keep unanswered cards due without blocking shutdown or Today generation.
- Daily Recall must test retrieval, not recognition or translation. Hide the target English item and cue it with its Chinese function, a genuine use condition and any meaning boundary needed to distinguish it; ask the learner to retrieve the English word, collocation or sentence-frame skeleton. Do not require a complete self-made sentence or exact word-for-word reconstruction: the core language and relationship are enough. Show the canonical item and card back only after the learner answers. A prompt that displays the target English and asks for a Chinese translation is invalid and must not produce recall evidence. Keep each item brief; total time follows the eligible items actually due. After each valid attempt, append the result to that card and update `last_recall`, `last_result`, `recall_stage` and `next_recall`.
...
1. Act as the LN905 mentor. Read `99_学习情况记录/teach/LN905 Exam Playbook.md`, the cross-session catalog `99_学习情况记录/teach/LN905 Skill Bank.md`, its linked atomic cards, the canonical task plan, the linked end-to-end record, weekly exam topic, source ledger, latest complete output, newest Friday samples and teacher feedback. Reuse the newest unfinished unit before selecting another. The task line and link must already exist; material generation never creates another checkbox.
...
10. In every guided record, write exactly one hidden mentor-owned block between `student-os:mentor-brief:start/end` before the init prompt. Fix `今日 principal`, `重点技能 IDs`, `起点证据`, `完整产出`, `端到端教学链`, `允许支架`, just-in-time prompted skills/language, independent-only observation opportunities, hint ladder, feedback priority, completion evidence, writeback rule and stop boundary. The assistant may choose only the next 5–10 minute move inside this chain. Intermediate map, plan or sentence work writes progress/evidence but never closes the task.
11. When introducing a sentence pattern or procedure, prescribe same-day deliberate practice as `explain function and cue → supported discrimination or imitation → varied production → integration into an exam-style paragraph/summary`. Write capability evidence to the matching `LN905 Skill Bank.md` record; upsert reusable language items as linked cards and set a first shutdown recall only when they meet the production-language eligibility rule above. A `new` skill reaches `guided` only when all four steps have evidence; same-day prompted success never counts as independent. During guided drafting, do not silently wait for useful taught sentence patterns, vocabulary/collocations or writing principles to appear. Immediately before a naturally suitable sentence or paragraph decision, tell the learner the function, why the item fits here and a small set of reusable frames or word choices, then require the learner to build the sentence; never provide the finished answer. Record that use as `guided`. Reserve silent observation for an explicit independent/complete simulation, a Friday/timed output, or a later mentor-designated unaided check after supported use. One unprompted use reaches `independent-1`; `stable` requires two unprompted uses on different material including one timed or Friday sample. Do not manufacture callback blanks, interrupt for irrelevant items or create another checkbox.
...
14. Give every guided LN905 unit's Markdown record an append-only block between `student-os:learning-log:start/end`. Before every teaching reply, append the learner's exact input, AI teaching/diagnosis, current end-to-end stage, the single next meaningful action and continuation decision, plus any just-in-time support used and any later independent-only observation evidence. Preserve earlier entries and resume from them across days. If the write fails, report that the learning record was not saved.
...
- Today contains exactly one opening-recall block and one shutdown-recall block. Each contains every eligible production-language card due at that checkpoint exactly once, with no sampling, and no procedural capability card; neither contains a checkbox or creates a calendar event. Every surfaced card exists once, has a unique `card_id`, and carries parseable recall dates; a repeated run does not duplicate a due item or learning-log entry.
