...
- Select ordinary LN905 work adaptively from the latest independent output. Vocabulary remains a parallel baseline and does not count as the day's guided part; normally schedule at most one highest-value guided LN905 part rather than automatically pairing Listening and Reading. If the meaning/evidence map is inaccurate, train the relevant input route; if the map is usable but the written output is weak, train the shared Writing node directly. If the same shared Writing node remains weak in two comparable outputs, prioritize it in the next two guided parts.
