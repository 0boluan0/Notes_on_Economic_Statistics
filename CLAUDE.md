...
- Whole-output feedback must make a recurring sentence problem understandable at the exact point where it is corrected. Use a canonical Skill Bank frame only when it genuinely reduces future writing effort; then, in the same correction row or comment, show the sentence job, the learner's concrete failure, the frame link, the frame itself, when to use it and its meaning boundary. If a direct source-faithful rewrite is more natural, use it without inventing or forcing a frame; a separate frame index must never substitute for inline explanation. Treat existing frames as revisable hypotheses, not authorities: test the frame against the real question, source relationship and learner sentence job, then keep, rewrite, merge or replace it rather than forcing the correction into an ill-fitting card. When a frame is used, show `problem -> frame -> source-based slot filling -> final sentence` in that order, and require the final sentence to instantiate the displayed frame. Read all corrected sentences as continuous prose: avoid reusing the same surface frame at short range unless the repetition is intentional parallelism, and make each body-paragraph ending turn its evidence into a writer-led claim linked to the thesis. Use explicit high-contrast foreground/background pairs for learner-facing correction blocks; never rely on font colour alone. When relevant teacher or course PDFs exist, show the exact applicable marking dimension in the same row or immediately above that paragraph, distinguish PDF wording from the AI comparison, and never present an unfilled class checklist as individual teacher feedback. Then upsert the same card with the independent evidence and, if a useful sentence job recurs across real or calibrated prompts, the smallest reusable variant and a source-faithful four-topic example. Do not prompt it before another independent/timed attempt, and do not create a separate drill or checkbox.
