...
- The former LN905 capability sequence in `99_学习情况记录/学习计划/LN905 Listening and Reading Practice.md` is paused from 2026-08-09. Today must not schedule its tasks or generate new artifacts until a replacement design has been discussed and explicitly confirmed. Formal timed submissions and hard deadlines remain active in their separate sources.
- Treat lexical access, material comprehension, argument organization and English sentence production as four distinct layers. Diagnose and teach one layer at a time; do not make a full lecture or three-paper pack, structure recovery, evaluation and a long blank-page response the entry requirement for ordinary practice.
...
- Do not invent a replacement LN905 sequence or timetable before the diagnostic discussion is complete. Once confirmed, keep one canonical checkbox per real practice unit; any derived vocabulary preparation remains inside that task rather than becoming another checkbox.
...
- Every AI-guided LN905 Listening HTML remains the durable review record. Keep an append-only block between `student-os:ai-review:start/end`; on every checkpoint turn, write the timestamped user-field snapshot, AI teaching/feedback, user answer or revision, and gate decision back into that session HTML before replying. Preserve all earlier entries and the existing localStorage key/fields; do not append an exact duplicate of the latest checkpoint snapshot. If the write fails, say that the review record was not saved.
