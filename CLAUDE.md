...
- Every guided record contains exactly one mentor-owned block between `student-os:mentor-brief:start/end`. It fixes today's principal, starting evidence, practice sequence, allowed support, any silent interleaved observation, feedback priority, completion evidence and stop boundary. The teaching assistant may append the learning log and close the canonical session task when the prescribed output is complete, but it must not edit the mentor brief or the cross-session skill ledger.
- Maintain the cross-session skill ledger in `99_学习情况记录/学习计划/LN905 Listening and Reading Practice.md`. A newly taught sentence pattern or procedure receives same-day deliberate practice from explanation through a varied application and an exam-integrated output. Later suitable tasks silently test one or two due skills among new material; do not name or hint at the observed skill before the learner's first output. Same-day prompted success is not mastery. Mark a skill stable only after at least two unprompted uses in different materials, including a timed or Friday course sample when available; otherwise keep it in circulation without creating a duplicate checkbox.
