...
- Calibrate guided Paper B units to the user's real packets. Until newer independent tests justify recalibration, use three academically written extracts adapted from credible scholarly sources, with total body length at least the longer observed packet (currently about 1,650 words); keep individual extracts in the observed academic-test range (currently about 425–610 body words), and make the language and source relationships at least as demanding as the Gender and Social Media tests. Do not substitute a short fact bank or partial pack.
