...
- Every LN905 Listening or Reading task gets a 15-minute `词灵生词预习` calendar block immediately before the main block. The prep block is derived from the same canonical task and never gets a second checkbox. Practice sessions use vocabulary from that session's material; timed submissions use only topic vocabulary and must not reveal the test material in advance.
...
- Unless a canonical LN905 task explicitly says `完整模拟`, make it AI-in-the-loop rather than a full solo attempt followed by grading. Use two closed-loop checkpoints: after a small independent evidence/structure attempt, AI diagnoses it and asks at most three targeted questions without revealing a model answer; the user answers and revises, and AI explicitly confirms when to draft. After the draft, AI gives criteria-linked feedback and one high-value revision target, then verifies the user's revision. These checkpoints remain inside the one canonical task and never create extra checkboxes.
- Every AI-guided LN905 Listening HTML is also the durable review record. Keep an append-only block between `student-os:ai-review:start/end`; on every checkpoint turn, write the timestamped user-field snapshot, AI diagnosis/questions, user answer or revision, and gate decision back into that session HTML before replying. Preserve all earlier entries and the existing localStorage key/fields; do not append an exact duplicate of the latest checkpoint snapshot. The final checkpoint payload includes notes, map, response, revision and reflection. Chat history and browser localStorage are working context only and do not satisfy this record. If the write fails, say that the review record was not saved.
- For a task explicitly marked `完整模拟`, preserve exam validity: the user receives no AI help during the timed attempt. AI joins only after time expires for diagnosis, feedback and one targeted revision.
