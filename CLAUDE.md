...
- Whole-output feedback must not leave a recurring, exam-usable sentence correction only in the essay record. Link it to the canonical Skill Bank card used by Daily Recall/Anki; if the learner does not know when or how to use that frame, upsert the card in the same post-output writeback with a clear sentence job, a genuine use condition, its meaning boundary and one source-faithful example. When an unprompted independent or timed output provides a natural opportunity but the learner cannot produce a usable, meaning-accurate sentence, promptly add or refine the smallest reusable frame in that same scaffold bank for recall and just-in-time use in the next guided unit. Record the attempt as independent evidence, do not prompt the frame before another independent/timed attempt, and do not create a separate drill or checkbox.
