...
- Whole-output feedback must make a reusable sentence problem understandable at the exact point where it is corrected. In the same correction row or comment, show the sentence job, the learner's concrete failure, the canonical Skill Bank frame link, the frame itself, when to use it and its meaning boundary; a separate frame index must never substitute for this inline explanation. Treat existing frames as revisable hypotheses, not authorities: test the frame against the real question, source relationship and learner sentence job, then keep, rewrite, merge or replace it rather than forcing the correction into an ill-fitting card. Show `problem -> frame -> source-based slot filling -> final sentence` in that order, and require the final sentence to instantiate the displayed frame. Use explicit high-contrast foreground/background pairs for learner-facing correction blocks; never rely on font colour alone. When relevant teacher or course PDFs exist, show the exact applicable marking dimension in the same row or immediately above that paragraph, distinguish PDF wording from the AI comparison, and never present an unfilled class checklist as individual teacher feedback. Then upsert the same card with the independent evidence and, if needed, the smallest reusable variant and a source-faithful four-topic example. Do not prompt it before another independent/timed attempt, and do not create a separate drill or checkbox.
