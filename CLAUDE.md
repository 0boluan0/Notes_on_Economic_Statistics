...
- Add one embedded active-recall step after daily shutdown and one at the next daily opening. Select one newly learned/used card at shutdown, otherwise the earliest due card; use a fresh context and let the learner answer before showing the card back. After correct recall, advance `+1, +3, +7, +14, +30` days; partial/incorrect returns next day; no answer is not failure. Append the result and next date to that card. This remains a 2–3 minute managed note step, including on Sunday, never a task, checkbox, deadline or calendar block, and never an automatic mastery-state change or revival of the old standalone `+1/+3/+7` review queue.
