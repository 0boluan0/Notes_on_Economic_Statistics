...
> - Both linear search and binary search will be formally compared in this lesson.
...
<!-- bilingual-en:start -->
- Some functions were too fast in the last lecture
- `time.time()` has insufficient resolution
- `perf_counter()` is better suited for measuring short code segments
<!-- bilingual-en:end -->
...
<!-- bilingual-en:start -->
*2. First, simple numeric functions: constant vs linear*
<!-- bilingual-en:end -->
课堂先用两个函数热身：
<!-- bilingual-en:start -->
...
- `compound(invest, interest, n_months)`: may exhibit linear growth depending on which parameter changes
<!-- bilingual-en:end -->
...
- Not every input change affects complexity
<!-- bilingual-en:end -->
...
<!-- bilingual-en:start -->
*3. Timing list functions: input size shifts from 'values' to 'list length'*
<!-- bilingual-en:end -->
随后课堂把输入类型切到 list。
<!-- bilingual-en:start -->
...
- `n` 是 `len(L)`
<!-- bilingual-en:start -->
- `n` is not the value of an element itself
- `n` is `len(L)`
<!-- bilingual-en:end -->
...
你得先定义“输入规模”。
<!-- bilingual-en:start -->
This point is crucial because one of the easiest places to get stuck in complexity analysis is defining what 'input size' means.
<!-- bilingual-en:end -->

### 4. linear search vs binary search：同一任务，不同增长阶
<!-- bilingual-en:start -->
*4. Linear search vs binary search: same task, different growth rates*
<!-- bilingual-en:end -->
这节课最重要的对比例子之一是查找元素。
<!-- bilingual-en:start -->
...
<!-- bilingual-en:start -->
- Linear search eliminates at most one element in the worst case
- Binary search halves the candidate space each time
<!-- bilingual-en:end -->
...
- 再看那一半的中点
<!-- bilingual-en:start -->
- Looking at the midpoint
- Deciding whether to keep the left or right half
- Then looking at the midpoint of that half
<!-- bilingual-en:end -->
...
<!-- bilingual-en:start -->
*7. `all_binary_numbers(N)`: exponential growth becomes genuinely daunting*
<!-- bilingual-en:end -->
...
这一步非常重要，因为它提醒你：
<!-- bilingual-en:start -->
This step is crucial because it reminds you that:
<!-- bilingual-en:end -->

- 有些问题不是“实现写得差”
- 而是任务本身的输出规模就决定了下界非常大
<!-- bilingual-en:start -->
- Some problems are not slow because the implementation is poor
- The size of the required output itself imposes a very large lower bound
<!-- bilingual-en:end -->

### 8. 从 timing 转向理论语言：order of growth
<!-- bilingual-en:start -->
*8. Moving from timing to theoretical language: order of growth*
<!-- bilingual-en:end -->
...
- 我们想比较输入变大时，增长趋势是什么
<!-- bilingual-en:start -->
- We do not want to record only the number of seconds on one particular machine
- We want to compare how runtime grows as the input becomes larger
<!-- bilingual-en:end -->

这就是 order of growth 的作用。
<!-- bilingual-en:start -->
That's where order of growth comes in.
<!-- bilingual-en:end -->
...
<!-- bilingual-en:start -->
- Big O only gives an upper bound
- This upper bound might be very loose
- Theta expresses an asymptotically tight bound
<!-- bilingual-en:end -->

也就是说，Theta 不是随便找个长得更快的函数就完事，而是要抓住真正同阶的增长。
<!-- bilingual-en:start -->
That is, Theta isn't just about finding some faster-growing function; it's about capturing the true asymptotic growth.
<!-- bilingual-en:end -->

### 10. 定义 `n` 代表什么，比写符号更重要
<!-- bilingual-en:start -->
*10. Why defining `n` is more important than writing symbols*
<!-- bilingual-en:end -->
这节课里老师反复追问：
<!-- bilingual-en:start -->
