...
\begin{bmatrix}
 1&3 \\ 2&7
\end{bmatrix}
$$的逆矩阵$A^{-1}$,就是在寻找$A^{-1}A=I$.那么就讲A和I拼接起来.得到一个增广矩阵(augmented matrix):
$$
\begin{bmatrix}
 1&3&1&0 \\ 2&7&0&1
\end{bmatrix}
$$而后使用消元法(行上的消元法.)将左侧的A变为单位矩阵(identity matrix).变完之后右侧就称为了A的逆矩阵
$$
\begin{bmatrix}
 1&0&7&-3 \\ 0&1&-2&1
\end{bmatrix}
...
同时还能进行更近一步的拆分,使A=LDU.D是对角矩阵(diagonal matrix). DU=上边的U,是A的进一步拆解.
