# **Chapter 5: Inferences about [[Population Growth and Economic Development|Population]] [[Mean-Variance Portfolio Optimization|Mean]] Vector (总体平均向量的推论)**

## **1. Testing Hypotheses about the [[Mean-Variance Portfolio Optimization|Mean]] Vector (均值向量的假设检验)**

• Focus on testing $H_0: \mu = \mu_0$ against $H_1: \mu \neq \mu_0$.
• Utilizes the [[Hotelling Model|Hotelling]]’s $T^2$ statistic:
$$
T^2 = n (\bar{X} - \mu_0)' S^{-1} (\bar{X} - \mu_0)
$$

where $\bar{X}$ is the sample [[Mean-Variance Portfolio Optimization|mean]] vector, and $S$ is the sample covariance matrix.
...
## **2. [[Confidence Level|Confidence]] Intervals and Regions (置信区间与置信区域)**

• [[Confidence Level|Confidence]] regions for the population [[Mean-Variance Portfolio Optimization|mean]] $\mu$ are ellipsoidal, defined by:
$$
n (\bar{X} - \mu)' S^{-1} (\bar{X} - \mu) \leq \frac{p(n-1)}{n-p} F_{p, n-p}(\alpha)
$$

• [[Simultaneous Equations Model|Simultaneous]] confidence intervals for individual components $\mu_1, \mu_2, \dots, \mu_p$:

$$
\bar{X}_i \pm \sqrt{\frac{p(n-1)}{n-p} F_{p, n-p}(\alpha)} \sqrt{\frac{S_{ii}}{n}}
$$

## **3. [[Hotelling Model|Hotelling]]’s $T^2$ and [[Likelihood Ratio Test|Likelihood]] Ratio Tests ([[Hotelling Model|Hotelling]]'s $T^2$ 与似然比检验)**

• [[Hotelling Model|Hotelling]]’s $T^2$ can be derived as a [[Likelihood Ratio Test|likelihood ratio test]].
• [[Likelihood Ratio Test|Likelihood]] ratio statistic $\[[Delta|Delta]]$:
$$
\[[Delta|Delta]] = \left( \frac{|\hat{\Sigma}|}{|\hat{\Sigma}_0|} \right)^{n/2}
$$

where $\hat{\Sigma}$ is the sample covariance matrix under $H_1$ and $\hat{\Sigma}_0$ under $H_0$.
• Relationship between $T^2$ and $\[[Delta|Delta]]$:
$$
\[[Delta|Delta]]^2 / n = (1 + \frac{T^2}{n-1})^{-1}
$$

## **4. Bonferroni Method for Multiple Comparisons (多重比较的Bonferroni方法)**
...
## **5. Large Sample Approximations (大样本近似)**
...
# **1. Testing Hypotheses about the [[Mean-Variance Portfolio Optimization|Mean]] Vector**

## 1.1**假设检验的目标 (Objective of Hypothesis Testing)**
...
### 1.2.1**[[Hotelling Model|Hotelling]]’s $T^2$** 统计量的设定
...
$$
其中
• $\displaystyle \overline{X} = \frac{1}{n}\sum_{j=1}^n X_j$ 是样本均值 (Sample [[Mean-Variance Portfolio Optimization|Mean]])；
• $s^2 = \tfrac{1}{n-1}\sum_{j=1}^n \bigl(X_j - \overline{X}\bigr)^2$ 是样本方差 (Sample [[Variance Decomposition|Variance]])；
• 在样本量不算太大且总体正态的情况下，该统计量$t$服从自由度为$(n-1)$的$t$分布，从而可以用来做区间估计或假设检验(如单样本$t$检验)。

但是矩阵没有除法,只有求逆,所以构造$T^2$来进行统计推断

**[[Hotelling Model|Hotelling]]’s $T^2$** 统计量定义为：
$$
T^2
...
$$
其中：
• $\bar{\mathbf{X}}$ 是样本均值向量 (Sample [[Mean-Variance Portfolio Optimization|Mean]] Vector)：
$$
\bar{\mathbf{X}} = \frac{1}{n} \sum_{i=1}^n \mathbf{X}_i
$$
• $\mathbf{S}$ 是样本协方差矩阵 (Sample Covariance [[Matrix Inverse|Matrix]])：
$$
\mathbf{S} = \frac{1}{n-1} \sum_{i=1}^n (\mathbf{X}_i - \bar{\mathbf{X}})(\mathbf{X}_i - \bar{\mathbf{X}})’
$$

### 1.2.2**[[Hotelling Model|Hotelling]]’s $T^2$** 统计量的特性
...
检验统计量是基于 [[Hotelling Model|Hotelling]]’s $T^2$ 的形式：
$$
T^2 = n (\hat{\psi} - \psi_0)^T (DSD’)^{-1} (\hat{\psi} - \psi_0),
$$
其中：
...
#### 证明
  ...
  下面用较为简略但核心的思路，说明在零假设 $H_0: \mu = \mu_0$ 成立且样本来自多元正态分布 $N_p(\mu,\Sigma)$ 时，为什么 [[Hotelling Model|Hotelling]] $T^2$ 统计量会与一个 $F$ 分布相联系，并具体呈现
...
在检验 $H_0: \mu = \mu_0$ 时，[[Hotelling Model|Hotelling]] $T^2$ 统计量定义为
$$
T^2
= n (\overline{X} - \mu_0)’S^{-1}(\overline{X} - \mu_0).
$$
...
其中 $W_p$ 是 [[Wishart Distribution|Wishart 分布]]。
...
Z \sim N_p(0, \Sigma).
$$
[[Hotelling Model|Hotelling]]’s $T^2$ 统计量可以写成：
$$
T^2 = Z^T S^{-1} Z.
$$
进一步，将样本协方差矩阵 $S$ 表示为 [[Wishart Distribution|Wishart]] 分布的形式：
$$
...
$$
T^2 = \frac{Q}{\text{Trace of Inverse [[Wishart Distribution|Wishart]] [[Matrix Inverse|Matrix]]}}.
$$
**6. 分布转化为 F 分布**
...
[[Hotelling Model|Hotelling]] $T^2$ 统计量的转换公式为：
$$F = \frac{T^2 \cdot (n-p)}{p \cdot (n-1)},$$
...
# **2. [[Hotelling Model|Hotelling]]’s $T^2$ and [[Likelihood Ratio Test|Likelihood]] Ratio Tests**

和前面的T是等价的.

[[Hotelling Model|Hotelling]]’s $T^2$ 统计量可以通过 **[[Likelihood Ratio Test|似然比检验]]（[[Likelihood Ratio Test|Likelihood Ratio Test]], LRT）** 的方法推导出来。这一部分主要讨论了 $T^2$ 与似然比检验的关系，以及如何通过似然比统计量进行假设检验。

**似然比统计量：**
$$
\[[Delta|Delta]] = \frac{\max_{\Sigma} L(\mu_0, \Sigma)}{\max_{\mu, \Sigma} L(\mu, \Sigma)},
$$
...
定义：
$$
\[[Delta|Delta]]^{2/n} = \frac{|\hat{\Sigma}|}{|\hat{\Sigma}_0|},
$$

称为 **Wilks’ Lambda**。
• 当 $\[[Delta|Delta]]^{2/n}$ 过小时，意味着 $H_0: \mu = \mu_0$ 不太可能成立。

**[[Likelihood Ratio Test|似然比检验]]：**
对于显著性水平 $\alpha$，如果：
$$
\[[Delta|Delta]] = \left(\frac{|\hat{\Sigma}|}{|\hat{\Sigma}_0|}\right)^{n/2} < c_\alpha,
$$

则拒绝原假设 $H_0: \mu = \mu_0$。

• $c_\alpha$ 是 $\[[Delta|Delta]]$ 的分布在显著性水平 $\alpha$ 下的分位数。似然比检验有它自己的分布,不属于平常见到的分布,所以应该不考

通过代数推导，可以得到：

$$
\[[Delta|Delta]]^{2/n} = \frac{1}{1 + \frac{T^2}{n-1}}.
$$

---

# **3. [[Confidence Level|Confidence]] Intervals and Regions**

**置信区域的定义 (Definition of [[Confidence Level|Confidence]] Regions)**
...
• $\frac{p(n-1)}{n-p} F_{p, n-p}(\alpha)$ 是由 $F$ 分布确定的置信区域参数 ([[Confidence Level|Confidence]] Region [[Parameter Identification|Parameter]])。

• 置信区域是一个以样本均值 $\bar{\mathbf{X}}$ 为中心的椭球 (Ellipsoid)。

**置信区域的几何含义 (Geometric Interpretation of [[Confidence Level|Confidence]] Regions)**

• $\mu$ 落在置信区域内的概率为 $1-\alpha$。
• 椭球的形状由协方差矩阵 $\mathbf{S}$ 决定，半轴的长度与样本协方差和自由度相关。

**同时置信区间 ([[Simultaneous Equations Model|Simultaneous]] [[Confidence Level|Confidence]] Intervals)**
...
• $\bar{X}_i$ 是样本均值的第 $i$ 个分量 (The $i$-th component of the sample [[Mean-Variance Portfolio Optimization|mean]] vector)。
• $S_{ii}$ 是样本协方差矩阵的对角元素 (Diagonal element of the sample covariance matrix)。

**检查均值向量是否在置信区域内 (Checking If $\mu_0$ Lies Within the [[Confidence Level|Confidence]] Region)**
...
**线性组合的置信区间 ([[Confidence Level|Confidence]] Intervals for [[Linear Combination|Linear]] Combinations)**
...
对于多元均值的 $1-\alpha$ 置信区间，[[Hotelling Model|Hotelling]] $T^2$ 的置信椭球可以通过以下公式给出：
...
---
# 4. [[Simultaneous Equations Model|Simultaneous]] [[Confidence Level|Confidence]] interval
...
和Simultaneous [[Confidence Level|Confidence]] interval有相同的目标,是两种方法.
...
**目标：** 控制全家错误率（Family-Wise [[Error Correction Model|Error]] Rate, FWER），确保对每个线性组合 $a’\mu$ 构造的置信区间同时有效。
...
**Bonferroni 同时置信区间公式 (Bonferroni [[Simultaneous Equations Model|Simultaneous]] [[Confidence Level|Confidence]] Intervals)**
...
**Bonferroni 方法与 $T^2$ 方法的比较 (Comparison to [[Hotelling Model|Hotelling]]’s $T^2$)**
...
• [[Hotelling Model|Hotelling]]’s $T^2$ 更适合用于全局显著性检验，特别是在多个变量间协方差矩阵影响显著时。

**大样本情形的近似 (Large-Sample Approximation)**

当样本量较大 ($n \to \infty$) 时：

1. [[Hotelling Model|Hotelling]]’s $T^2$ 统计量逐渐趋近于 $\chi^2$ 分布：
$$
T^2 \sim \chi^2_p
$$
