...
- 似然比统计量（likelihood ratio statistic）记作 $\Delta$：
...
• 在样本量不算太大且总体正态时，该统计量 $t$ 服从自由度为 $(n-1)$ 的 $t$ 分布，可用于区间估计与假设检验（如单样本 $t$ 检验）。

但在矩阵情形下没有“除法”运算，只有逆矩阵运算，因此构造 $T^2$ 做统计推断。
...
经过线性变换后，$T$ 统计量的分布不变，因为线性缩放的效应会在协方差矩阵与偏移向量之间相互抵消。这里的线性变换是单位变换一类的缩放，而非原始变量之间的线性组合。

#### [[Linear Combination|线性组合]]

若对原变量做线性组合形成新矩阵后再做 $T^2$ 检验，分布会改变，因为矩阵维度与结构发生了变化。
