...
# 3. 置信区间与置信区域（Confidence Intervals and Regions）

**定义（置信区域, Confidence Region）：**

对于 $p$ 维正态分布 $N_p(\mu, \Sigma)$ 的样本，假设 $\mathbf{X}_1, \mathbf{X}_2, \dots, \mathbf{X}_n$ 是样本观测值，样本均值向量为 $\bar{\mathbf{X}}$，样本协方差矩阵为 $\mathbf{S}$。在显著性水平 $\alpha$ 下，$p$ 维均值向量 $\mu$ 的 $100(1-\alpha)\%$ **置信区域**定义为满足以下条件的所有 $\mu$：
