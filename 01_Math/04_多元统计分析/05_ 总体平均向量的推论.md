# 第5章：总体平均向量的推论（Inferences about Population Mean Vector）

## 0. 回忆用

- 核心任务：检验 $H_0:\mu=\mu_0$ 对 $H_1:\mu
eq\mu_0$。
- 核心统计量：Hotelling 的 $T^2$ 统计量（[[Hotelling Model|Hotelling's $T^2$]]）。

$$
T^2 = n (ar{X} - \mu_0)' S^{-1} (ar{X} - \mu_0)
$$

其中 $ar{X}$ 是样本均值向量（sample mean vector），$S$ 是样本协方差矩阵（sample covariance matrix）。

在 $H_0$ 下：

$$
T^2 \sim rac{p(n-1)}{n-p} F_{p,n-p}
$$

## 1. 置信区间与置信区域（Confidence Intervals and Regions）

总体均值向量 $\mu$ 的置信区域是椭球，满足：

$$
n (ar{X} - \mu)' S^{-1} (ar{X} - \mu) \leq rac{p(n-1)}{n-p} F_{p,n-p}(lpha)
$$

各分量的同时置信区间（simultaneous confidence intervals）可写为：

$$
ar{X}_i \pm \sqrt{rac{p(n-1)}{n-p} F_{p,n-p}(lpha)}\,\sqrt{rac{S_{ii}}{n}}
$$

## 2. $T^2$ 与似然比检验（Likelihood Ratio Test）

- $T^2$ 可以由似然比检验（[[Likelihood Ratio Test|Likelihood Ratio Test]]）推导得到。
- 似然比统计量（likelihood ratio statistic）记作 $\[[Delta|Delta]]$：

$$
\[[Delta|Delta]] = \left( rac{|\hat{\Sigma}|}{|\hat{\Sigma}_0|} ight)^{n/2}
$$

并且有：

$$
\[[Delta|Delta]]^{2/n} = \left(1 + rac{T^2}{n-1}ight)^{-1}
$$

## 3. Bonferroni 多重比较（Bonferroni Method）

当比较数量较少时，Bonferroni 区间通常比 $T^2$ 区间更短：

$$
ar{X}_i \pm t_{n-1}\!\left(rac{lpha}{2m}ight)\sqrt{rac{S_{ii}}{n}}
$$

## 4. 大样本近似（Large-Sample Approximation）
...
# 1. 均值向量的假设检验（Testing Hypotheses about the Mean Vector）
...
### 1.2.1 [[Hotelling Model|Hotelling]] 的 $T^2$ 统计量设定
...
### 1.2.2 [[Hotelling Model|Hotelling]] 的 $T^2$ 统计量性质
...
**定义（Definition）：置信区域（[[Confidence Level|Confidence]] Regions）**
...
• $\bar{X}_i$ 是样本均值的第 $i$ 个分量 （样本均值向量的第 $i$ 个分量）。
• $S_{ii}$ 是样本协方差矩阵的对角元素 （样本协方差矩阵的对角元素）。
...
• $a$ 是一个非随机向量 （非随机向量）。
• $a’\bar{\mathbf{X}}$ 是线性组合的估计值 （$a’\mu$ 的估计值）。
...
**Bonferroni 方法与 $T^2$ 方法比较（Comparison to [[Hotelling Model|Hotelling]]’s $T^2$）**
