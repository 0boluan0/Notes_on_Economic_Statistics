---
aliases: []
tags:
  - teach
  - learning/annotated-bibliography
date: "2026-08-10"
source_type: "teach"
topic: "Annotated Bibliography"
---

# Annotated Bibliography resources

## Course requirements

- [[07_Programme/01_LN905_LSE-language-class/PDF/02_Academic-Writing/Week-2-Gender/2026 WEEK TWO Assignment Booklet LN905.pdf#page=25|Week 2 Assignment Booklet · Annotated Bibliographies]]
  LN905 的正式定义与内容要求：每则 100–300 词，包含来源背景与受众、内容与核心论点、方法、相关性、优势、弱点、局限和 intended use；语言正式、客观且格式一致。
- [[07_Programme/01_LN905_LSE-language-class/PDF/02_Academic-Writing/Week-2-Gender/2026 WEEK TWO Assignment Booklet LN905.pdf#page=28|Week 2 Assignment Booklet · submission task]]
  核验正式提交数量为三篇文本。
- [[07_Programme/01_LN905_LSE-language-class/PDF/02_Academic-Writing/Week-2-Gender/Week 2 Lesson 4.pdf|Week 2 Lesson 4]]
  提供 student extracts 与教师对 description、evaluation、usefulness 和 intended use 的颜色拆解。
- [[07_Programme/01_LN905_LSE-language-class/01_LN905_note/04_Academic Writing#Annotated bibliography：summary + evaluation + intended use|Academic Writing note · annotated bibliography]]
  仓库内的已核对课程总结和可复用句子结构。

## Current essay and source pool

- [[07_Programme/01_LN905_LSE-language-class/02_LN905_essay/LN905 Essay|LN905 Essay project]]
  提供 essay question、暂定 thesis、正式里程碑与十篇来源在论证中的角色。
- [[07_Programme/01_LN905_LSE-language-class/02_LN905_essay/Annotated Bibliography/00_文章梗概索引|Annotated Bibliography 文章梗概索引]]
  用于从十篇候选来源中选择能形成清楚对话的三篇。
- [[07_Programme/01_LN905_LSE-language-class/02_LN905_essay/Annotated Bibliography/01 Lessmann et al. (2015) - Benchmarking credit-scoring algorithms|Lessmann et al. (2015) summary draft]]
  本课的 worked example；训练如何把 benchmark 的问题、方法、发现、局限与 essay 用途压缩进一则 annotation。
- [[07_Programme/01_LN905_LSE-language-class/02_LN905_essay/Annotated Bibliography/02 Khandani et al. (2010) - Consumer credit-risk ML models|Khandani et al. (2010) summary draft]]
  本课的独立迁移练习；重点区分算法复杂度、数据丰富度和假设驱动的经济收益。
- [[07_Programme/01_LN905_LSE-language-class/02_LN905_essay/Annotated Bibliography/03 Rudin (2019) - Stop explaining black-box models|Rudin (2019) summary draft]]
  用于训练理论/评论型来源的评价，并与实证 benchmark 建立 challenge/qualify 关系。

## Gaps

- Moodle 或教师说明中采用的具体 referencing style 尚未在项目笔记中确认。
- Reference 是否计入单篇 100–300 词尚未确认；在核验前，annotation 正文本身应独立满足词数要求。
