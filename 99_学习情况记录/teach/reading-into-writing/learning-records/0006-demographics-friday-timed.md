---
aliases:
  - Demographics Friday Paper B end-to-end review
tags:
  - teach
...
# Demographics Friday Paper B｜timed attempt → 赛后端到端重建

> [!summary] 这次完整产出
> 保留今天的独立成文，再用原题面和全部三篇 extracts 完成 `question-led source audit → meaning/evidence map → top-down thesis/paragraph jobs → 整篇诊断 → 一次 targeted whole-output revision → Skill Bank 写回`。整条链只属于今天原有的 timed attempt，不产生第二个 checkbox。

**独立原稿**：[[07_Programme/01_LN905_LSE-language-class/00_inbox/week3reading-into-writing|今日 Demographics 成文]]（只读）。

**正式试卷**：[[07_Programme/01_LN905_LSE-language-class/PDF/04_Paper-B-Reading-into-Writing/Week-3-Demographics/Demographics Extracts.pdf|Demographics Extracts]]（原题面与三篇 extracts）。

**Canonical task**：[[99_学习情况记录/学习计划/LN905 timed submissions#提交清单|Demographics Reading timed attempt]]。该 task 记录 timed attempt 已完成；Moodle 提交仍由 [[99_学习情况记录/deadlines|hard obligation]] 独立核验。

<!-- student-os:mentor-brief:start
阶段：正式 complete simulation 的赛后 teaching/reconstruction。
今日 principal：以三篇正式 extracts 和独立成文为证据，定位 source reading/map 与 shared Writing tree 的真实断点，再把同一题端到端重建。
重点技能 IDs：PB-02、PB-03、PB-04、AW-15。
起点证据：今日成文使用了 Ezeh et al.、Bloom et al. 和 Jones，有清晰总答案、三个 paragraph jobs 和 conclusion；但已核验的 Skill Bank 证据显示多处新增 cost/causality、消除条件与 scope，并把跨文限定压平成“immigration 无效”。语言细节不是本次首要问题。
完整产出：原成文保持不变；一张可核验的三源 meaning/evidence map；一份 task → provisional answer → paragraph jobs → evidence roles 的 top-down plan；whole-output diagnosis；最多一次 learner 完成的 targeted whole-output revision；Skill Bank 真实证据写回。
端到端教学链：原题拆解 → 三源 claim/evidence/mechanism/scope/use 审计 → agreement/explanation/qualification/tension 关系图 → thesis 与 paragraph jobs → evidence roles/selection → 整篇对照反馈 → targeted revision → evidence writeback。
允许支架：可用中文解释、原 extracts 必要短段、source inventory、关系箭头、worked contrast、function cue、structure cue 和短模型；不得代写整篇 revision。
静默 callback IDs：AW-16。适用机会：赛后显式重建 `task → provisional answer → paragraph jobs → evidence roles`；原稿没保留 planning 过程，不能倒推为 independent。
提示层级：先说明当前信息转换和写作用途 → function cue → structure cue → 基于原材料的短对比；同一局部问题最多要求一次修正，然后回到整篇。
反馈优先级：task fulfilment → source meaning/ownership/certainty/scope → source relationships/synthesis → top-down organization/completion → recurring language；一次性拼写和小语法不得劫持任务。
完成证据：原题、extracts 和原成文均只读保留；map 保留每源的 proposition/evidence/scope 及跨文关系；plan 由题目和总答案控制；revision 修复最高价值的整篇问题；Skill Bank 只记录 independent/guided/incorrect/not observable。
写回规则：canonical timed task 根据独立完整成文关闭，不等待赛后 revision；hard deadline 只由 Moodle/receipt 关闭。本记录不建 checkbox。一次 timed 自主正确调用最多到 independent-1；stable 仍需另一份不同材料证据。
停止边界：完成一次 targeted whole-output revision 与 Skill Bank 写回后，明确回复“本次 Friday Paper B 端到端复盘完成”；不得启动 Listening、未来训练或改动 Inbox 原文件。
student-os:mentor-brief:end -->

## 给新 Codex 助教的 init prompt

你是本次 LN905 Demographics Friday Paper B 的赛后执行助教。只在当前 Academic vault 和本记录中工作，不创建 worktree，不新增 checkbox，不安排日程，不处理 Listening into Writing。

先完整读取本文件、`99_学习情况记录/teach/LN905 Exam Playbook.md`、`99_学习情况记录/teach/LN905 Skill Bank.md`、`07_Programme/01_LN905_LSE-language-class/PDF/04_Paper-B-Reading-into-Writing/Week-3-Demographics/Demographics Extracts.pdf` 和今天 course Inbox 中的独立成文。试卷与 Inbox 均只读，禁止移动、删除或改写；不得用搜索到的原论文、摘要或你自己改编的材料代替考试 extracts。

材料齐全后，第一条教学回复先给紧凑 orientation card：

- `这次完整产出`：保留 timed 原稿，完成可核验三源 map、top-down reconstruction 和一次 targeted revision；
- `为什么走完整链`：目的是分清 source acquisition、source transformation、writing decision 和 English production 究竟断在哪里，不是集中挑错；
- `考试流程`：question → three-source map → thesis/paragraph jobs → evidence roles → essay → prioritized check；
- `完成标准`：直接回答题目、准确使用三源、写清来源关系、保留 ownership/certainty/scope，并完成约 600 词全文；
- `你已经会`：能使用三份来源、建立整体立场、组织三个功能段并完成 conclusion；
- `当前节点`：先用原试卷审计原稿的 source map，不从漂亮句子或语法开始。

之后每轮只做一个约 5–10 分钟的有意义动作。先读完整原稿和所有 sources 再判断，不得逐句纠缠措辞。只在 meaning、source ownership、certainty、scope、causality 或 evidence relationship 被实质改变时立即修正，其余语言问题放到 whole-output feedback 批量处理。需要 source detail 时，每次只展示完成当前动作所需的原文段落。

每次教学回复前，在 `student-os:learning-log:start/end` 之间追加：时间、learner 原始输入或 Inbox 证据链接、AI 教学/反馈、当前端到端节点、本轮唯一下一动作、continuation decision；静默 callback 只记录真实证据。只追加，不覆盖，不重复最新一轮；写入失败必须明确说明记录未保存。

## Independent timed evidence（只读）

- [[07_Programme/01_LN905_LSE-language-class/00_inbox/week3reading-into-writing|今日独立成文]]
- [[07_Programme/01_LN905_LSE-language-class/PDF/04_Paper-B-Reading-into-Writing/Week-3-Demographics/Demographics Extracts.pdf|原题面与三篇 extracts]]

## Three-source meaning/evidence map

### Immigration｜guided reconstruction

- **Extract One｜Ezeh et al.**：把 immigration 视为经济发达、低生育国家的一项重要政策选择；同时保留所需人数可能达到数亿、对工作年龄人口与被抚养人口比例的作用，以及来源国 brain drain / brain gain 影响不明确等限制。
- **Extract Three｜Jones**：不是直接反对 immigration，而是按国家条件缩小其适用范围。中国人口规模使净移民不太可能大到足以显著改变人口趋势；日本和韩国若能在政治上接受大规模移民，人口趋势可能受到明显影响，但所需人数很多，政府与公众支持不足，而且不能保证完全停止下降。
...
### 2026-08-15 16:34 BST｜开始读取老龄化政策材料

- **Learner 原始输入**：`OK继续`
- **AI 教学 / 反馈**：learner 已确认可以继续。上一轮已证明其理解题目要求：政府是行动者，答案必须同时处理 population ageing 与 low fertility，并说明政策限制；不再要求 learner 改写审题句。现在从题目出发读取 Bloom et al.，先找政府针对老龄化能采取的行动、行动要解决的问题，以及材料保留的限制。
- **当前端到端节点**：题目分析已完成，进入三份材料的定向阅读；本轮只处理 ageing，不提前处理 fertility、移民关系或英文句子。
- **本轮唯一下一动作**：阅读本轮展示的 Extract Two 必要原文，用中文写三点：政府可以做什么；主要解决什么问题；为什么不能把一种政策写成万能方案。预计 5–10 分钟。
- **Continuation decision**：等待 learner 的 ageing 三点判断；下一轮只核对是否忠实保留原文行动、作用与限制，成立后再读取 low-fertility 材料。

### 2026-08-15 16:42 BST｜补回三篇材料的快速分工

- **Learner 原始输入**：`所以假设我刚刚才拿到这个题目和三篇文章,分析完问题之后我就进入这一段了吗?`
- **AI 教学 / 反馈**：不应从审题直接跳进 Extract Two 的具体段落。真实考场中，审题之后先快速扫三篇的标题、小标题和段落开头，判断每篇主要能回答 ageing、low fertility、政策限制中的哪些部分；完成这一步后，才回到相关段落仔细读取。上一轮因 AI 已在后台完成材料分工，直接展示 Extract Two，隐藏了 learner 在考场必须亲自完成的一步。
- **当前端到端节点**：题目分析已完成；退回补齐“三篇材料快速分工”，尚未开始逐段细读。
- **本轮唯一下一动作**：仅根据三篇标题和可见主题，用一句话写出 Extract One、Two、Three 各自可能为题目提供什么；同一篇可服务不止一部分。预计 3–5 分钟。
- **Continuation decision**：等待 learner 完成三篇分工；核对后再按题目需要定向细读，不按文章顺序平均阅读。

### 2026-08-15 16:56 BST｜确认“端到端”的 learner-owned 含义

- **Learner 原始输入**：`对的对的对的,我说端到端就是让你这样子带着我来做的`
- **AI 教学 / 反馈**：learner 明确确认：端到端不仅要求最后得到完整 essay，还要求按真实考场顺序亲自完成每一个关键判断。AI 可以解释、提供当前需要的材料和支架，但不能在后台替 learner 完成三篇分工、材料整理、总答案、段落作用或证据选择后再从后续节点开始。该定义已同步补入仓库基础指令。
- **当前端到端节点**：题目分析完成；正在进行三篇材料的第一次快速分工，尚未进入任何一篇的细读。
- **本轮唯一下一动作**：根据三篇标题，用中文判断 Extract One、Two、Three 各自可能帮助回答题目的哪一部分；同一篇可有多个作用。预计 3–5 分钟。
- **Continuation decision**：等待 learner 的三篇分工；只核对分工方向，随后进入 learner 自己选择和提取材料的阶段，不再替其跳步。

### 2026-08-15 17:09 BST｜三篇材料首次分工与关键误读定位

- **Learner 原始输入**：
  > Extract 1,前面说了一点那个,就是气候变化最近就很严重,然后不是气候变化,是那个人口生育率的这些问题最近很严重,然后给了一些就是比较宽泛的建议来提升低生育率的。然后还有什么减税的政策,就是减少那种有孩子家庭的负担,或者引进移民,就这两条。第二篇主要是针对老龄化的,它首先说老人可以意味着有经验的工作者,然后让他们重新去干活,对他们的身体健康也好,也可以就是少花钱让他们有产出,对这个养老金和健康保险基金都好。让他们更加独立,花更少的钱。然后第三篇是探讨一些特别,就是主要是东亚的这些国家提升生育率的办法。他说移民是没有效果的,对于东亚这些国家都没有效果。然后说单一的效果没有用,它需要这种对养孩子的这些补贴,对于增加工作机会,就工作生活平衡,然后给每个小孩每个月付钱。然后工作的时间会更长,然后减轻这些妇女的这个性别的角色什么的。然后这些,并且说明这些政策要同时用,不能只用一个。
- **AI 教学 / 反馈**：三篇分工方向成立：Extract One 提供较宽的低生育政策与移民选项；Extract Two 主要处理 ageing；Extract Three 提供东亚低生育的具体条件、政策组合与移民限制。需要立即修正三处实质性误读：Extract Two 把“老年人继续贡献经验”和“健康、独立、长期照护成本”写成了工作会直接改善健康并降低支出的因果链，原文没有证明这条链；Extract Three 并未说移民对所有东亚国家都无效，而是区分中国的规模限制与日本、韩国在大规模且政治上可接受时的条件性作用；长工时是阻碍 work–family balance 的问题，不是作者建议增加的政策。`no one policy can deal with all the unresolved issues` 表示单一政策不足以解决全部问题，不等于每项政策单独都毫无作用。
- **当前端到端节点**：三篇材料的第一次快速分工已完成；进入有针对性的细读，先修复 Extract Three 的移民判断。
- **本轮唯一下一动作**：阅读本轮展示的 Extract Three 移民段，只用两句中文分别写清中国与日本/韩国的情况，并保留原文的条件和限制。预计 5–10 分钟。
- **Continuation decision**：等待 learner 的两国组判断；准确后再读取 Extract One 的移民段，比较两份来源怎样共同支持一个有限判断。

### 2026-08-15 17:13 BST｜Extract Three 移民判断第一次修正

- **Learner 原始输入**：`中国太大了,要靠移民解决那个数量不可能. 日本如果引进移民那个民族主义不会好的.`
- **AI 教学 / 反馈**：中国部分抓住了人口规模使净移民难以达到足以显著改变人口趋势的数量，但 learner 用“不可能”代替原文 `unlikely`，增强了确定性。日本部分意识到社会接受问题，却新增了原文没有提出的“民族主义”解释，并遗漏原文的另一半：对日本和韩国而言，大规模移民如果在政治上可接受，可能明显影响人口增减；限制在于所需规模很大、公众与政府支持不足，而且日本每年接收 200,000 人仍不足以完全阻止人口下降。
- **当前端到端节点**：Extract Three 的移民段正在细读；中国的规模限制方向成立，日本/韩国的条件性作用与政治、数量限制尚未完整保留。
- **本轮唯一下一动作**：learner 只重写日本/韩国一句，必须同时保留“在什么条件下可能有用”以及“现实中为什么受限”；不得加入民族主义等原文未给出的原因。预计 3–5 分钟。
- **Continuation decision**：本局部问题只再修正一次；若仍不完整，直接提供准确短模型并进入 Extract One 与 Extract Three 的移民关系比较，不继续循环同一句。

### 2026-08-15 17:17 BST｜移民条件修正成立并进入跨文比较

- **Learner 原始输入**：`1.大规模接受移民可能有用2.但是政府和民众很难接受`
- **AI 教学 / 反馈**：learner 已同时保留 Extract Three 对日本/韩国移民政策的两面判断：大规模移民可能有用，但政府与公众的接受度构成现实限制。表达虽省略“所需人数很大且不足以完全停止下降”的补充限定，但当前命名关系已成立；按规则不继续要求改写同一句。AI 提供完整短模型后推进到 Extract One 与 Extract Three 的关系判断。
- **当前端到端节点**：Extract Three 的移民条件已修复；开始比较两份来源如何共同界定移民在低生育国家中的作用。
- **本轮唯一下一动作**：阅读 Extract One 的移民段，并结合刚完成的 Extract Three 判断，用一句中文说明 Jones 怎样补充或限制 Ezeh et al. 的移民主张，以及这对题目答案意味着什么。预计 5–10 分钟。
- **Continuation decision**：等待 learner 的跨文关系句；只核对两文关系与 writer conclusion，不纠缠措辞，成立后将该关系写入三源材料整理并继续其他政策主题。

### 2026-08-15 17:21 BST｜移民跨文关系提供短模型并停止局部循环

- **Learner 原始输入**：`1 说移民是一个可选的政治选项,但是有研究者说这个用移民来解决生育率问题需要的移民数量太大了. 2. 直接反对`
- **AI 教学 / 反馈**：learner 准确提取了 Extract One 的两面信息：immigration 是政策选项，但所需规模可能极大；然而把 Jones 判断为“直接反对”仍改变了两文关系。若 Jones 直接反对，他不会保留日本/韩国在大规模移民且政治上可接受时 `could make a real difference` 的可能性。准确关系是 Jones 具体说明并限制 Ezeh et al. 的一般主张：移民可能有部分作用，但有效性与可实施性取决于国家规模、人数和政治接受度。按同一局部问题最多一次修正的规则，不再要求 learner 重写；准确短模型已写入 Three-source meaning/evidence map，并标记为 guided。
- **当前端到端节点**：移民这一组材料关系已在支架下完成；继续细读低生育政策，比较 Extract One 的常见家庭支持措施与 Extract Three 对其效果有限的解释。
- **本轮唯一下一动作**：阅读本轮展示的 Extract One 与 Extract Three 相关段落，用中文说明两文共同认可哪些政策，以及 Jones 为什么认为这些政策目前效果有限。预计 5–10 分钟。
- **Continuation decision**：等待 learner 的低生育政策关系判断；先核对政策、机制与限制，成立后写入同一张材料整理，再处理 ageing 部分。

### 2026-08-15 17:34 BST｜确认低生育政策分组并解除词汇阻塞

- **Learner 原始输入**：`1. 给生育的家庭经济支持,改善工作文化更wlb. 2. 不懂,因为最关键的entrenched不认识`
- **AI 教学 / 反馈**：learner 已准确把两文共同认可的措施归为家庭经济支持与改善 work–life balance / 工作文化；未逐项抄写，材料选择方向成立。第二问的阻塞来自 lexical access，而不是政策推理：`entrenched` 在 `entrenched cultural, institutional and familial arrangements` 中表示长期形成、根深蒂固且难以改变，并非“最关键”。Jones 的解释是补贴、产假等表层措施不足以单独改变长工时、女性承担主要育儿责任、教育压力与城市住房压力等深层安排，因此目前提升生育率的效果有限。
...
- [[99_学习情况记录/teach/LN905 Exam Playbook#Paper B｜进考场后怎么做|Paper B top-down procedure]]
- [[07_Programme/01_LN905_LSE-language-class/PDF/04_Paper-B-Reading-into-Writing/Reading into Writing.pdf|Paper B Introduction]]
