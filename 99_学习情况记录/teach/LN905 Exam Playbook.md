...
- 不知道下一句该调用哪个骨架：打开 [[99_学习情况记录/teach/LN905 Skill Bank/Cards Index#当前练习回收｜下次直接调用|当前练习回收的句子骨架]]，先按句子工作选择，再用当前材料填入内容。
...
- 整篇反馈发现可复用的造句问题时，不能只留下改好句：同次写回必须链接并更新原子骨架卡，补齐句子工作、真实适用条件、意义边界和四主题内例句；若现有卡已覆盖则追加证据和变体，不新建同义卡。该骨架进入 Daily Recall/Anki，并在下一份 guided unit 的自然位置即时调用；不另开句子练习或 checkbox。
