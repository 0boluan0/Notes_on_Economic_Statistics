...
- 整篇反馈发现可复用的造句问题时，必须在出现该错误的同一条逐句批注里依次显示：这句要完成的工作 → 学习者具体错在哪里 → 下次何时调用哪个骨架及其意义边界，并直接显示骨架、链接原子卡。单独的骨架汇总或索引不能代替这一处即时解释。若现有卡已覆盖则追加证据和变体，不新建同义卡；骨架进入 Daily Recall/Anki，并在下一份 guided unit 的自然位置即时调用，不另开 drill 或 checkbox。
