...
```mermaid
flowchart LR
    L["Listening：从时间流中听取、选择并记下信息"] --> M["共同 meaning/evidence map"]
    R["Reading：从三篇 extracts 中阅读、选择并连接信息"] --> M
    M --> A["明确写作任务与总答案"]
    A --> S["选材与全文结构"]
    S --> P["段落推理、synthesis 与 evaluation"]
    P --> E["英文实现"]
    E --> C["限时完成与检查"]
```
...
- 整篇反馈发现造句问题时，先判断骨架是否真的能减少下一次写作负担。能时，必须在该错误的同一条逐句批注里按 `问题 → 骨架 → 来源填槽 → 由该骨架生成的完整句` 显示，并写清句子工作、具体失败、使用条件与意义边界；最终句必须真正由当行显示的骨架生成。直接、来源忠实的改写更自然时，就直接修正，不新造也不强套骨架。骨架不是固定答案：先用真实题目、来源关系和本次自然输出审查它，再决定保留、改写、合并或替换；不能为了复用旧卡反向扭曲句子。把所有改好句连成连续正文再读一遍：同一表面骨架不能在短距离重复，除非是有意的平行结构；每个主体段的段尾必须把本段证据转成作者自己的判断并回扣 thesis。批改块使用明确的浅色底与深色字，不能只靠绿色、红色等字体颜色传达信息。单独的骨架汇总或索引不能代替错误现场解释。有对应老师或课程 PDF 时，还要在同一行或紧邻段落处标出适用的正式评分维度，严格区分 PDF 明文与 AI 对照判断，不能把未填写的全班 checklist 写成个别教师反馈。只有一个好用的句子工作在真实题或校准模拟中反复出现时，才新增或细化骨架；若现有卡能经最小改写覆盖则追加证据和变体，重复卡应合并，不新建同义卡。骨架进入 Daily Recall/Anki，并在下一份 guided unit 的自然位置即时调用，不另开 drill 或 checkbox。
