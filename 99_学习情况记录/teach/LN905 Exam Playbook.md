...
- 整篇反馈发现可复用的造句问题时，必须在出现该错误的同一条逐句批注里按 `问题 → 骨架 → 来源填槽 → 由该骨架生成的完整句` 显示，并写清句子工作、具体失败、使用条件与意义边界；最终句必须真正由当行显示的骨架生成，不能把一个只在意思上相关的旧卡贴在旁边。骨架不是固定答案：先用真实题目、来源关系和本次自然输出审查它，再决定保留、改写、合并或替换；不能为了复用旧卡反向扭曲句子。批改块使用明确的浅色底与深色字，不能只靠绿色、红色等字体颜色传达信息。单独的骨架汇总或索引不能代替错误现场解释。有对应老师或课程 PDF 时，还要在同一行或紧邻段落处标出适用的正式评分维度，严格区分 PDF 明文与 AI 对照判断，不能把未填写的全班 checklist 写成个别教师反馈。若现有卡能经最小改写覆盖则追加证据和变体；重复卡应合并，不新建同义卡。骨架进入 Daily Recall/Anki，并在下一份 guided unit 的自然位置即时调用，不另开 drill 或 checkbox。
