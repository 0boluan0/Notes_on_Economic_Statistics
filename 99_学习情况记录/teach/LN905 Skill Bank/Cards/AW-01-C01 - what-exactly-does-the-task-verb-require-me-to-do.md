---
student_os: ln905-skill-card
card_id: AW-01-C01
card_title: "What exactly does the task verb require me to do?"
parent_skills:
  - AW-01
card_type: decision
learned_on: 2026-08-20
recall_stage: capability-only
next_recall:
last_recall:
last_result: not-applicable
---

# `What exactly does the task verb require me to do?`

> [!question] 正面
> `What exactly does the task verb require me to do?`

> [!answer]- 反面
> **功能**：把题目的 command 变成可执行动作。
>
> **适用时机**：第一次读 essay question 时，先判断要 explain、evaluate、compare、discuss 还是 argue。
>
> **边界**：不能只抓 topic noun；同一主题在不同 task verb 下需要完全不同的证据和结构。
>
> **例句/例子**：`Evaluate` requires a judgement against criteria, not a list of advantages and disadvantages.
>
> **可替换形式或操作提示**：先圈 task verb，再用一句中文说出“阅卷人要我做的判断”。
>
> **课件来源**：[[07_Programme/01_LN905_LSE-language-class/PDF/02_Academic-Writing/Week-1/Description Essay Question & Rationale.pdf|Description Essay Question & Rationale]]；[[07_Programme/01_LN905_LSE-language-class/PDF/02_Academic-Writing/Week-1/LN905 Week one materials 2026.pdf|LN905 Week one materials 2026]]；[[07_Programme/01_LN905_LSE-language-class/PDF/02_Academic-Writing/Week-1/Week One Lesson One.pdf|Week One Lesson One]]

## 学习记录

- 2026-08-20｜课件回溯初始化｜已从已上 LN905 课件整理成卡片并加入 Recall 队列；尚未留下主动回忆或独立使用证据，不视为掌握。
- 2026-08-21｜开场 Recall｜学习者回答“evaluate 有预设正确答案，预设后面的说法都对”。这与课程定义不符：`evaluate` 要求依据 criteria 与 evidence 作出 judgement，并不预设命题正确；记为 incorrect，次日再回忆。
- 2026-08-21｜系统纠正｜这是一张程序性 task-analysis 卡，不属于每日语言 Recall；取消次日 Recall。上条回答只保留为端到端教学参考，不作为每日 Recall 表现。
