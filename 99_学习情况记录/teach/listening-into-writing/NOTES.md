...
# Teaching notes: LN905 Listening into Writing

## 学习偏好
- 2026-08-13 起，普通练习恢复端到端单位：一次完整材料必须继续到完整 critical summary 与整篇反馈/修订后才完成，可跨天。AI 仍逐步教学并保留分层诊断，但 map、句型、evaluation 不再单独成任务或完成状态。
- learner-facing 材料、示范和 callback 只使用 Social Media、Gender、Demographics、Climate Change。
...
- Listening 只在把时间流输入还原成 meaning/evidence map 之前拥有独立技能；map 之后在同一个 checkbox、Markdown 与助教会话中继续共用 W1–W9，直到完整成文。
...
- 停止生成 guided-practice HTML。每个端到端 unit 使用一个 Markdown 与一个 init prompt；同一 Codex 教学会话可跨天续做，完整成文与整篇修订后才停止。
...
- 材料理解、summary、evaluation 和英文实现仍分层诊断；当日容量不足时保存当前节点，次日续做同一条链，不另建短练。
...
- `词灵`保持独立；其余节点只在端到端 unit 内出现。正式无帮助模拟由周五测试承担。
...
- Week 1 原始笔记抓到了不少主要内容，也能完整记录讲者明确列出的四种方法；困难集中在没有显式编号的前半段论证结构。
