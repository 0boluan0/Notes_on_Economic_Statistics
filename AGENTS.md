...
- The user is in LN905 **Group 2**. During this course, the matching downloaded Moodle Group 2 timetable PDF is the authoritative fixed-class input; the Apple `LSE` subscription is supplemental and may contain deadlines without class meetings. Never infer “no class” from an empty `LSE` day.
...
- Keep **input acquisition drills** and **top-down Writing drills** distinct. A Listening or Reading input drill ends with a usable meaning/evidence map and does not force prose. Once that map is usable, a Writing drill starts from `question/task → provisional answer/thesis → paragraph functions → evidence roles/selection → prose`; it never uses source-by-source summary or exhaustive extraction as a proxy for learning to write.
- A genuine Listening or Reading input practice normally runs as a same-day pair: first finish the input map, then—while the material is still fresh—start one adjacent Shared Writing part that uses that map. The two parts keep separate canonical checkboxes, Markdown records and init prompts. If the day cannot hold both, prefer a standalone Writing drill with a supplied compact fact bank or short academic extract; never present that short support material as Reading practice.
- Calibrate genuine Reading input practice to the user's real Paper B packets. Until newer independent tests justify recalibration, use three academically written extracts adapted from credible scholarly sources, with total body length at least the longer observed packet (currently about 1,650 words); keep individual extracts in the observed academic-test range (currently about 425–610 body words), and make the language and source relationships at least as demanding as the Gender and Social Media tests. Shorter sources are allowed only inside a clearly labelled standalone Shared Writing drill or partial Writing exercise.
...
- Select ordinary LN905 work adaptively from the latest independent output. Vocabulary remains a parallel baseline. Normally select one training lane rather than pairing Listening and Reading modalities: either one standalone Writing part, or one same-day input → Shared Writing pair using the same material. If the meaning/evidence map is inaccurate, train the relevant input route and immediately reuse that map in the paired Writing part when capacity allows; if the map is already usable, train the weakest shared Writing node directly. If the same shared Writing node remains weak in two comparable outputs, prioritize it in the next two guided parts.
...
- The Student OS task is the **mentor**: it selects the capability, deliberate-practice dose, interleaved retrieval opportunity, evidence standard and stop point. A part-specific Codex task is only a **teaching assistant**: it grades the current response and assigns the next 5–10 minute move inside the mentor's prescription. It may not choose a different capability, redesign the sequence, schedule future work or declare long-term mastery.
- Every guided record contains exactly one mentor-owned block between `student-os:mentor-brief:start/end`. It fixes `今日 principal`, `主技能 ID`, starting evidence, output, deliberate-practice chain, allowed support, `静默 callback IDs`, `callback 适用机会`, `提示层级`, feedback priority, completion evidence, `技能状态写回规则` and stop boundary. The teaching assistant may append the learning log, write evidence and a legal transition only to the named Skill Bank IDs under that rule, and close the canonical session task when the prescribed output is complete; it must not edit the brief, select another skill or declare mastery beyond the rule.
- `99_学习情况记录/teach/LN905 Skill Bank.md` is the only cross-session state source for all learned academic/assessment-transferable LN905 capabilities and vocabulary. Course notes retain full explanations; the practice plan retains tasks only. Use IDs in `AW`, `PA`, `PB`, `PC`, `AI`, `DISC`, `PRON` and `LEX`. Exclude pure Everyday English, classroom administration and game rules. Backfill only taught material; downloaded Week 4 content remains inactive until the class occurs.
- Every important LN905 skill must complete one explicit loop `explanation → supported application → varied application → integration into a real exam-style output`; use a natural existing task or one function-grouped canonical skill clinic, never one checkbox per phrase. Legal states are `new`, `guided`, `independent-1`, `stable` and `repair`. Same-day prompted success reaches at most `guided`; one later unprompted use reaches `independent-1`; `stable` requires a second unprompted use on different material including one Friday/timed sample.
- Each guided part has one primary skill. The mentor may select any number of old skills as silent callbacks only when each is naturally applicable and due; there is no mechanical count target. Do not reveal callback IDs or hints before the learner's first complete output, do not add callback-only blanks or interrupt the principal, and record only `independent`, `guided`, `incorrect` or `not observable`. No natural opportunity is never failure. When a callback is missed, use only `function cue → structure cue → short contrast/model`, then return to the principal; batch minor wording, spelling and one-off grammar after the complete output.
...
- The canonical top-down procedure is `99_学习情况记录/teach/LN905 Exam Playbook.md`. Every guided part trains one named step in that playbook and must reconnect the learner to the whole exam workflow; never let a practice become a free-standing correction drill.
- Official LN905 assessment introductions, marking criteria and Academic Writing slides define the capability tree and success criteria; teacher feedback personalizes priority. When uncertain, inspect those course files instead of inventing a skill, structure or marking rule. Label symbols, time splits and other non-course methods as training recommendations. Each daily part must name its playbook node, exam location, concrete output, observable learning evidence and one integration action, so the sequence accumulates toward complete input-to-writing simulations.
- The first reply in every guided part gives a compact orientation card before the exercise: `今天在学` names one transferable capability in plain Chinese, `为什么` explains the real human writing problem it solves and ties it to the user's evidence, `考试位置` names exactly where and how it is used in the exam procedure, `学会的样子` states an observable finish condition, and `你已经会` identifies the relevant existing strength. Before asking for any action, explain how that action transforms the information in front of the learner and which next writing decision it unlocks; show a worked contrast or example when needed. “The teacher or criteria requires it” is not a sufficient explanation. This is a skill map, not a list of all future tasks. Every later reply briefly locates the learner on that same map; completion records one short exam cue in the form `遇到 X → 做 Y`.
