...
- Add one embedded active-recall block at each daily opening and shutdown. Each block must include **every** card due or overdue at that checkpoint; shutdown also includes every card newly learned or used since opening whose first recall is due that day. Never sample, cap or select only the earliest card. Use a fresh context for each item and let the learner answer before showing its back. After correct recall, advance `+1, +3, +7, +14, +30` days; partial/incorrect returns next day; no answer is not failure and remains due for the next recall checkpoint. Keep each item brief, but let the total time follow the number actually due. This remains one managed note block, including on Sunday, never a task, checkbox, deadline or calendar block, and never an automatic mastery-state change or revival of the old standalone `+1/+3/+7` review queue.
...
- The confirmed **Academic Writing guided replay** is now taught through one continuous, realistic end-to-end simulation rather than standalone blocks or spaced micro-tests. Use one exam-scale pack of three adapted academic extracts and one essay question; guide the learner from receiving the paper through question analysis, question-led reading, a three-source meaning/evidence map, provisional thesis, paragraph jobs and evidence roles, full draft, whole-output feedback/revision and a final submission check. This is `teaching practice`, not a no-help `完整模拟`: explain and scaffold the current stage, display necessary source passages, and let the learner make every writing decision and write the final prose. Keep all stages in one Markdown record and one task, and do not run the old `+1/+3/+7` standalone review queue. The mandatory credit-scoring assignment may progress in parallel during its dedicated class/drafting time; keep its teaching record, working draft and completion state separate from the Climate Change simulation, and save each corrected assignment passage promptly because it will become the submitted draft. Revisit earlier skills naturally inside the same complete chain, and finish only after the complete essay and revision.
