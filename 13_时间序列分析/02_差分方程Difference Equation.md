...
这其实就是在形式上写出了$y_t$的解，只不过这里有一个分数$\frac{1}{A(L)}$，需要我们展开或化简。**稳定性条件**（即所有特征根$\alpha_>1$）保证了$A(L)$在$|L|<1$的范围内可以展开为收敛的幂级数，从而使得$1/A(L)$有意义（这类似于$\frac{1}{1-x} = 1 + x + x^2 + \cdots$在$|x|<1$时成立）。
