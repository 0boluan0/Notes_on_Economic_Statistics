...
**两个术语：** 我们首先介绍两个将在本讲中用到的符号。

• **差分算子 $\Delta$：** 定义 $\Delta y_t \equiv y_t - y_{t-1}$。它表示变量在相邻两期之间的变化量。例如，如果$y_t$表示$t$期的GDP，那么$\Delta y_t$就是$t$期GDP相对于$t-1$期的增量。类似地，可以定义二阶差分 $\Delta^2 y_t \equiv \Delta(\Delta y_t)$，即：

$$\Delta^2 y_t = \Delta(y_t - y_{t-1}) = (y_t - y_{t-1}) - (y_{t-1} - y_{t-2}) = y_t - 2y_{t-1} + y_{t-2}.$$

二阶差分表示相邻两期差分的差分，常用于捕捉加速度或曲率等特性。

• **滞后算子 $L$：** 定义 $L^i y_t = y_{t-i}$。也就是说，$L$算子将时间序列向后“推迟”$i$期。例如，$L y_t = y_{t-1}$表示将序列滞后一期。教材第40页给出了滞后算子的一些性质（性质1-6）。有了滞后算子，我们可以简洁地表示差分，如$\Delta y_t = y_t - L y_t$。直观上，滞后算子帮助我们方便地引用过去的数值，而差分算子关注当前值与前一期值之差。
...
考虑时间序列分析中的经典分解模型，由趋势($T$)、季节($S$)和不规则($I$)三部分组成：

$$y_t = T_t + S_t + I_t.$$

举例来说，设 $T_t = 1 + 0.1t$（确定性趋势项），$S_t = 1.6 \sin(t\pi/6)$（季节周期项），$I_t = 0.7 I_{t-1} + \varepsilon_t$（不规则项），其中$\varepsilon_t$是纯随机扰动。可以看到，这三部分各自满足一定形式的差分方程：趋势项是线性的确定性差分关系，季节项满足周期性关系，不规则项是一个带随机扰动的一阶差分方程。

  

**差分方程的重要性：** 一般来说，**差分方程**用来表示一个变量的当前值是其自身滞后值、时间和其他变量的函数。在上述分解中，每个分量的递推关系都可以看作差分方程。在经济和金融时间序列中，很多模型都可以表示为变量与其滞后值之间的关系。时间序列计量经济学的核心任务之一，就是对包含随机成分的差分方程进行建模和估计。因此，理解差分方程对于分析和预测时间序列数据至关重要。
...
**一般形式：** 一个$n$阶**线性差分方程**（具有恒定系数）的形式可以写为：
...
其中，$a_0, a_1, …, a_n$为常数系数，$x_t$称为**外生驱动过程**(forcing process)。$x_t$可以是时间$t$的函数、其他变量当前或滞后的值，或者随机扰动（例如 $x_t = \sum_{i=0}^{\infty}\beta_i \varepsilon_{t-i}$ 表示一个移动平均的随机冲击序列）。

  

**差分算子表示：** 利用差分算子，上述方程(1)也可改写为：
...
其中 $\gamma = a_1 - 1$。这个形式将$y_t$关于自身滞后值的关系用$\Delta y_t$表示为一个包含$\gamma y_{t-1}$的方程。直观而言，如果$a_1$等于1，那么$\gamma=0$，差分方程中就没有$y_{t-1}$项，这种情况对应单位根过程；如果$a_1$不等于1，则$\Delta y_t$和$y_{t-1}$存在线性关系。
...
**定义：** 差分方程的**解**是指用${x_t}$序列（驱动项）和时间$t$的函数来表示$y_t$的方法（有时还需利用一些给定的$y_t$初始条件）。这类似于微积分中对微分方程求解——那里我们通过积分得到一个函数形式的解，这里通过迭代或其他方法得到$y_t$的显式表达。

  

**例1：** 考虑简单的一阶差分方程 $\Delta y_t = 2$，即$y_t - y_{t-1} = 2$。我们可以猜测其解为：

$$y_t = 2t + C,$$

其中$C$是一个任意常数。验证一下：$y_t - y_{t-1} = (2t + C) - [2(t-1) + C] = 2$，满足方程要求。因此$y_t = 2t + C$确实是方程$\Delta y_t=2$的通解。这里$C$通常通过初始条件确定。

  

**例2：** 再看不规则成分的模型 $I_t = 0.7 I_{t-1} + \varepsilon_t$。这个方程表示$I_t$受到上一期自身的0.7倍和当期随机冲击的影响。可以验证，该差分方程的一个解是：
...
这是一个由过去所有冲击的衰减加权和构成的解：当前的不规则成分等于以0.7为衰减因子的历次冲击累积效应。我们可以验证：如果将这个$I_t$代入方程右侧，利用滞后一个期的同样表达式，就能匹配$I_t$。这个解的直观意义是：由于$I_t$是AR(1)结构（自回归一阶），它的值等于所有过往冲击按衰减系数累计的结果。

  

上述两个例子中的解似乎都是**直接猜出来**的。那么有没有系统的方法来**求解**差分方程呢？接下来我们介绍求解差分方程的两种途径：迭代法和一种更通用的方法。
...
迭代法是通过逐步向前或向后推导，来构造差分方程的解的方法。我们先从简单情况入手。
...
这个公式分为三部分：第一项是由常数项$a_0$逐期累积产生的部分（一个几何级数和），第二项是初始值$y_0$经过$t$期后衰减或放大的部分，第三项是由历次随机冲击累积而来的部分。上述推导中，**教材第10页有个小排版错误**，请以我们推导的结果为准。

  

正向迭代给出了在已知$y_0$时$y_t$的明确表达，但如果初始条件未知或不完整，我们还需要考虑反向迭代的方法。

  

**反向迭代（向前推回初值）：** 假设初始条件$y_0$未知，我们依然可以将方程向过去迭代：

• 对于$t=0$时刻，我们有

$$y_0 = a_0 + a_1 y_{-1} + \varepsilon_0,$$

其中$y_{-1}$可以被看作再往前一period的值。将公式(3)中$t$替换为$t+1$，然后用上式替换$y_0$，相当于向前推1期，可得：

\[

y_t = a_0 \sum_{i=0}^{t} a_1^i \;+\; \sum_{i=0}^{t} a_1^i \, \varepsilon_{t-i} \;+\; a_1^{\,t+1} y_{-1}. \tag{4}

\]

这里比(3)式多出来最后一项，其中出现了$y_{-1}$。

• 类似地，如果我们对未知初始值向前迭代$m$期（即假定我们追溯到$y_{-m-1}$仍未知），则可以得到：

\[

y_t = a_0 \sum_{i=0}^{\,t+m} a_1^i \;+\; \sum_{i=0}^{\,t+m} a_1^i \, \varepsilon_{t-i} \;+\; a_1^{\,t+m+1} \, y_{-m-1}. \tag{5}

\]

当向过去无限推移（$m \to \infty$）时，如果某些条件满足，我们期望能够消除对未知初始值的依赖，得到一个仅由已知量表示的解。

  

**注1：** 如果$|a_1| < 1$（即系数$a_1$的绝对值小于1，表示具有均值回复的稳定过程），让$m \to \infty$，则$a_1^{,t+m+1} y_{-m-1}$这一项中的系数$a_1^{,t+m+1}$将趋于0，于是(5)式变为：

$$

y_t = \frac{a_0}{,1 - a_1,} ;+; \sum_{i=0}^{\infty} a_1^i , \varepsilon_{t-i}, \tag{6}

$$

这时$y_t$的解不再依赖遥远过去的初值，而是完全由模型的参数和冲击决定。其中$\frac{a_0}{1 - a_1}$是一个常数解部分，$\sum_{i=0}^{\infty} a_1^i \varepsilon_{t-i}$是冲击的累积效应部分。这实际上就是前面例2中的结果。可以将(6)代回原方程(2)验证其正确性。

  

**注2：** 即使初始条件未知，我们也可以进一步推断：(2)式在$|a_1|<1$条件下**任意**一个解都可以表示为：
...
其中$A$是任意常数。（7）式表示了(2)式的**通解**：它由特解（后两项，其中不含$A$的部分）加上齐次解（$A a_1^t$项）组成。可以通过将(7)代入(2)式验证其确实是解。

  

**注3：** 当我们给定初始值$y_0$时，就可以利用$y_0$来确定通解中的常数$A$。【由于$y_0$对应$t=0$期，将$t=0$代入(7)：】

$$

y_0 = A \cdot a_1^0 + \frac{a_0}{,1 - a_1,} + \sum_{i=0}^{\infty} a_1^i , \varepsilon_{-i},

$$

注意$\sum_{i=0}^{\infty} a_1^i \varepsilon_{-i}$表示从$t=-\infty$到$t=-0$（即$t=0$前）的冲击累积。如果假设在$t<0$时刻没有冲击（或冲击均值为0），则这一项可以看作0。这样得到

$$A = y_0 - \frac{a_0}{,1 - a_1,}.$$
...
这与前面通过正向迭代得到的(3)式是一致的（可以证明(3)式和(8)式等价）。在(8)中，第一项是初始值贡献的**衰减项**（如果$|a_1|<1$，这部分会随着$t$增长而趋于0），后两项则是稳态部分和随机冲击部分。

  

**注4：** 如果$a_1$的绝对值大于或等于1（$|a_1| \ge 1$），则无法像(6)那样让$a_1^{,t+m+1}$项消失——也就是说，(5)式的解会永久依赖远期初始条件，(6)式不成立。然而，即便$|a_1| \ge 1$，（3）式的推导依然成立，只是这时解不会收敛于稳态。例如，当$a_1 = 1$时，（2）式表示$y_t = a_0 + 1 \cdot y_{t-1} + \varepsilon_t$，这等价于$\Delta y_t = a_0 + \varepsilon_t$。通过累加求和可以得到特殊的解：

$$y_t = a_0 , t + \sum_{i=1}^{t} \varepsilon_i + y_0,$$

其中$y_0$为初始值。可以看到$a_1 = 1$时，解包含一项随时间线性增加的漂移$a_0 t$和冲击的累积和——这实际上是单位根（随机游走）过程的特征。若$a_1 > 1$，解会发散（指数增长）；若$a_1 < -1$，解会在正负之间爆炸式震荡；$a_1 = -1$则导致结果在两值之间交替，处于所谓“亚稳态”。这些情况通常意味着模型不稳定，需要特别处理。
...
虽然迭代法直观且用于一阶差分方程时相当方便，但对于高阶差分方程，直接迭代会变得非常繁杂，代数运算的复杂性会迅速增长，使解题过程难以管理。在考试或实际分析中，我们需要一种更系统的方法来求解。

  

**思想：** 可以将差分方程的求解过程与微分方程类比。对微分方程，我们通常先求解对应的**齐次方程**，然后求一个**特解**，两者相加得到通解，并利用初始条件确定其中的常数。同样地，对于差分方程，我们也可以将其分为**齐次部分**和**特例部分**分别求解。

  

为说明这种通用方法，我们先考虑(2)式的齐次部分：
...
这个方程去除了常数项和随机扰动，仅保留了$y_t$对自身一阶滞后的依赖。方程(9)称为原差分方程的**齐次方程**，它的解称为**齐次解**（记作$y_t^h$）。显然，(9)有一个**平凡解**：$y_t = y_{t-1} = \cdots = 0$（所有时期$y$均为零）。但我们对非平凡解更感兴趣。
...
其中$A$是任意常数。代入(9)可以验证：左侧$A a_1^t$，右侧$a_1 y_{t-1} = a_1 (A a_1^{t-1}) = A a_1^t$，两边相等。于是$y_t^h = A a_1^t$就是齐次方程的通解（平凡解是$A=0$的特例）。这里$A$的任意性表明初始条件尚未纳入。

  

**关于稳定性：** 对于齐次解$A a_1^t$，$a_1$的大小决定了序列的稳定性：

• 如果$|a_1| < 1$，则当$t \to \infty$时，$a_1^t \to 0$，意味着$y_t^h$趋于0，序列稳定。

• 如果$|a_1| > 1$，齐次解将不稳定：若$a_1 > 1$，$y_t^h$会随着$t$增长发散至无穷大；若$a_1 < -1$，$y_t^h$会因符号交替并振幅增大而“爆炸式”震荡。

• 如果$a_1 = 1$，则$y_t^h = A$是一个任意常数（序列停留在某一水平，不发散也不收敛）。

• 如果$a_1 = -1$，则$y_t^h$在$A$和$-A$之间交替，属于边际稳定（既不趋于0也不发散，但在有限的两值之间震荡）。

  

以上我们已经通过迭代法找到了(2)式的一个特解(6)，即

$$y_t^p = \frac{a_0}{1 - a_1} + \sum_{i=0}^{\infty} a_1^i \varepsilon_{t-i}.$$

因为特解不唯一，所以称其为**一个**特解。将前面得到的齐次解和这个特解结合，就可以写出原方程(2)的**通解**。

  

**通解：** 差分方程的通解一般表示为“一个特解 + 所有齐次解的线性组合”。对于(2)式，一般通解可以表述为：

$$

y_t = y_t^p + y_t^h = \left[\frac{a_0}{1 - a_1} + \sum_{i=0}^{\infty} a_1^i \varepsilon_{t-i}\right] + A a_1^t,

$$

这正是(7)式所描述的结果。然后利用初始条件可确定$A$的值（如通过(8)式确定）。这个思路推广到高阶差分方程同样适用。

  

**求解线性差分方程的一般步骤：** 对于$n$阶线性差分方程，一个系统的解法包括以下四步：

1. **写出齐次方程并求齐次解：** 忽略外生项和常数项，只考虑$y_t$自身滞后的关系，求出$n$个线性独立的齐次解（$n$阶方程应有$n$个齐次独立解，对应$n$个待定常数）。

2. **找到一个特解：** 为包含外生项（例如常数项、时间趋势或随机扰动）的原方程猜测并求出其中一个特解。

3. **构造通解：** 将特解与齐次解的线性组合相加，得到包含$n$个任意常数的通解。

4. **利用初始条件消去任意常数：** 根据已知的初始值（通常需要给定$n$个初始条件），求解这些任意常数，从而得到特定解。

  

上述四步和微分方程的解法高度类似，只是将连续时间的导数变为了离散时间的滞后与差分。在实际求解时，第1步通常通过解**特征方程**完成，第2步往往需要一定的技巧猜测合适形式。下面通过一个二阶差分方程的例子来演示这一方法。
...
求其齐次解。假设解形式为$y_t^h = C \alpha^t$，代入得到特征方程（后面第4节将详细讨论）：

$$\alpha^2 - 0.9,\alpha + 0.2 = 0.$$

解这个方程可以找到两个特征根$\alpha_1 = 0.5, ; \alpha_2 = 0.4$，于是对应的两个齐次解为

$$y_{1t}^h = A_1 (0.5)^t, \qquad y_{2t}^h = A_2 (0.4)^t,$$

其中$A_1, A_2$是任意常数。【注意：一般求$n$阶齐次解的方法后面会介绍，这里直接给出结果。】

2. **特解：** 猜测一个特解形式。方程(10)右侧是常数3，没有随时间变化且无随机项，通常可以猜测特解为一个常数。代入验证：若令$y_t^p = 10$为常数解，则

左边$0.9 y_{t-1} - 0.2 y_{t-2}$对于常数序列都等于$0.9 \cdot 10 - 0.2 \cdot 10 = 7$，加上$a_0=3$正好得到10，符合$y_t=10$的假设。因此$y_t^p = 10$是方程(10)的一个特解。

3. **通解：** 将特解和齐次解相加，得到通解的一般形式：

$$y_t = A_1 (0.5)^t + A_2 (0.4)^t + 10,$$

其中$A_1, A_2$为待定常数。

4. **利用初值确定常数：** 假设已知初始条件$y_0 = 13$和$y_1 = 11.3$，将$t=0,1$代入通解以求$A_1, A_2$：

• 当$t=0$时：$y_0 = A_1 (0.5)^0 + A_2 (0.4)^0 + 10 = A_1 + A_2 + 10$。代入$y_0=13$，得到方程 $A_1 + A_2 = 3$。

• 当$t=1$时：$y_1 = A_1 (0.5)^1 + A_2 (0.4)^1 + 10 = 0.5 A_1 + 0.4 A_2 + 10$。代入$y_1=11.3$，得到 $0.5A_1 + 0.4A_2 = 1.3$。

联立以上两式求解$A_1$和$A_2$：从第一式$A_2 = 3 - A_1$，代入第二式得$0.5A_1 + 0.4(3 - A_1) = 1.3$，解得$A_1 = 1$，进而$A_2 = 2$。所以通解确定为

$$y_t = 1 \cdot (0.5)^t + 2 \cdot (0.4)^t + 10.$$

  

在这个例子中，我们成功应用了“四步法”求解了二阶差分方程。这种方法对于更高阶的线性差分方程同样适用：只是第1步找到齐次解会需要求解更高次的特征方程，第2步猜特解的难度也会随着$x_t$形式不同而不同，但整体框架是一致的。考试中，**熟练掌握这一框架十分重要**，因为它提供了解题的清晰思路：先齐次后特解，最后结合初始条件。
...
$$s_t = b + \beta p^__t + \varepsilon_t, \tag{12}$$_
