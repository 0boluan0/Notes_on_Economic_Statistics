...
*实例：资本资产定价模型（CAPM）可视为单因子模型的特例，其中所有股票收益只受一个市场因子影响，以及各自特有风险。*

**模拟考题：**设有单因子模型 $U_1 = 0.7\,F + \sqrt{0.51}\,Z_1$，$U_2 = 0.5\,F + \sqrt{0.75}\,Z_2$，其中共同因子 $F \sim N(0,1)$，特质因子 $Z_1, Z_2 \sim N(0,1)$ 互相独立且与 $F$ 独立。求 $U_1$ 与 $U_2$ 的相关系数。

**详细解答：**根据单因子模型的相关性公式，有 $Corr(U_1, U_2) = a_1 \times a_2$。这里 $a_1 = 0.7,\; a_2 = 0.5$，因此 
$$
Corr(U_1, U_2) = 0.7 \times 0.5 = 0.35 \,. 
$$ 
我们也可以通过协方差计算验证：$Cov(U_1, U_2) = 0.7 \times 0.5 \times Var(F) = 0.35$，且 $U_1, U_2$ 均为标准正态（方差1），故相关系数 = 协方差 = 0.35。

若改为两因子模型，相关系数则是各因子部分相关性的叠加。例如，若 $U_1 = b_{11}F_1 + b_{12}F_2 + \dots$，$U_2 = b_{21}F_1 + b_{22}F_2 + \dots$，则 $Corr(U_1, U_2) = b_{11}b_{21} + b_{12}b_{22} + \cdots$。

## Gaussian Copula 建模步骤
