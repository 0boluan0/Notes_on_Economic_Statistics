...
计算VaR常用的三种方法为：**方差-协方差法（参数法）**、**历史模拟法**和**蒙特卡罗模拟法**。它们各有原理和适用情形，下面分别介绍并比较其优缺点。
...
**方法小结：**方差-协方差法快但依赖分布假设，历史模拟法直观但受限于过往数据，蒙特卡罗法灵活精确但计算成本高。在实践中，风险管理常结合多种方法交叉验证。例如，日常风控用参数法快速估计，重大决策时辅以蒙特卡罗法深入分析。
...
选择置信水平反映机构风险偏好和用途：商业银行内部管理常用95%或97.5%等置信水平，国际大行如J.P. Morgan、美洲银行偏好95%，花旗银行用97.5%，美国信孚银行则采用99%【风险偏好越保守，选取越高置信度】。金融监管（如巴塞尔委员会）通常要求99%甚至更高（信用风险99.9%）的置信水平，以确保极高置信度下的损失也计提资本金。这体现了监管更谨慎的态度。

总体来说，提高置信水平会显著增加VaR。例如，同一组合在95%水平下的VaR可能远小于99%水平下的VaR，因为后者需要涵盖1%最极端损失。风险管理人员应根据管理目的选择适当水平：日常管理平衡效率可用95%，资本充足率评估则需99%甚至更高。
...
**ES（Expected Shortfall，预期损失）**也称**条件VaR（CVaR）**或**尾部期望损失**，是另一种风险度量，定义为在损失超过VaR条件下的平均损失。换言之，ES关心的是“最坏的$(1-\alpha)$的情况平均会损失多少”。举例来说，在99%置信水平下的10日ES是指未来10天内，处于最糟糕1%情形下的平均损失金额。
...
**一致性（Coherence）问题：**在风险度量理论中，一致性指满足一组合理性的公理（详见下一节）。ES被证明是**一致风险度量**，而VaR并不满足所有一致性要求（特别是次可加性）。这使得ES在理论上更受青睐，因为它不会像VaR那样可能违反分散化原则。例如，对于两个子组合，ES(A+B)总是≤ ES(A)+ES(B)，而VaR在某些罕见情况下可能出现组合风险>单独风险之和的反直觉结果。
...
1997年，Artzner等人提出了**一致性（Coherence）**风险度量的概念，认为合理的风险度量应满足以下公理：

1. **单调性（Monotonicity）**：若组合$X$的损失始终不高于$Y$（对所有状态$X\le Y$），则$X$的风险应不大于$Y$的风险。（亏损更大的组合风险值应更高）
2. **次可加性（Subadditivity）**：组合风险不应超过各部分风险之和：对于任意组合$X$和$Y$，有$\rho(X+Y) \le \rho(X) + \rho(Y)$。这体现了分散化有助于降低风险。
3. **正齐次性（Positive Homogeneity）**：投资规模成比例扩大，风险值同比例放大：对于$\lambda>0$，$\rho(\lambda X) = \lambda \rho(X)$。
4. **平移不变性（Translation Invariance）**：在组合中增加确定收益（或损失）$K$（例如持有$K$金额的现金），会使风险度量相应减少（或增加）同样的量：$\rho(X+K) = \rho(X) - K$。（注意这里损失取正，所以加$K$收益等于风险减$K$）

**VaR 的不一致性：**VaR一般满足单调性、正齐次、平移不变性，但**不保证满足次可加性**。绝大多数情况下VaR是次可加的（分散化降低VaR），但在某些分布下可能出现$\text{VaR}(X+Y) > \text{VaR}(X)+\text{VaR}(Y)$。典型例子是具有极端不对称风险的组合：

*示例：*两个独立贷款A和B，各有1年97.5% VaR为100万美元（意味着每笔贷款一年内有2.5%概率损失超过100万）。如果这两笔贷款极端情况下损失可能叠加，组合AB的一年97.5% VaR可能达到1100万美元，远大于各自VaR之和200万，**违反了次可加性**。这个构造的例子说明VaR可能不满足分散化原则，在风险度量理论上被视为一大缺陷。

**CVaR/ES 的优势：**CVaR（ES）满足上述所有一致性公理，包括次可加性，因而是一致风险度量。对于前述贷款组合例子，计算2.5%尾部的预期损失，会发现组合的ES等于或小于各自ES之和，体现了风险分散效应。总体而言，ES不仅规避了VaR的理论问题，还提供了尾部损失信息，因此在衡量极端风险时更可靠。

**光谱风险度量：**为了广泛刻画风险厌恶程度，Acerbi等提出**光谱风险度量（Spectral Risk Measure）**概念。光谱风险度量将损失分布的各个分位损失按照某种**权重函数**加权求和：
$$
...
**小结：**VaR因不满足次可加性在理论上受到批评，不过由于其直观性仍被广泛使用；CVaR/ES作为VaR的改进度量，满足一致性且提供尾部风险信息，逐渐在监管和实践中获得青睐；光谱风险度量提供了更灵活的框架来设计风险度量，以适应不同的风险厌恶偏好，同时确保次可加性。

## 7. VaR 分解（边际VaR、成分VaR、递增VaR）及欧拉分解法

在投资组合层面，风险管理者常需要知道**每个组成部分对整体VaR的贡献**以及增减某部分对VaR的影响。这催生了VaR的分解概念，包括**边际VaR**、**成分VaR**和**递增VaR**，它们在组合风险度量和风险预算中非常有用。
