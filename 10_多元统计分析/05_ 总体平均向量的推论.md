...
那么：
$$
T^2_{p, n-1} = (\sqrt{n}(\bar{X} - \mu_0)’) \left( \frac{\sum_{j=1}^n (\mathbf{X}_j - \bar{\mathbf{X}})(\mathbf{X}_j - \bar{\mathbf{X}})’}{n-1} \right)^{-1} (\sqrt{n}(\bar{X} - \mu_0))= N_p(0, \Sigma)’ \left[\frac{1}{n-1} W_{n-1}(\Sigma)\right]^{-1} N_p(0, \Sigma),
$$

表示：
• $T^2$ 统计量结合了：

1. **多元正态分布** $N_p(0, \Sigma)$，
2. **Wishart 分布** $W_{n-1}(\Sigma)$。

这表示 $T^2$ 统计量通过均值向量 $\bar{\mathbf{X}}$ 与协方差矩阵的联合估计来检验总体均值是否等于假设的均值 $\mu_0$。这一结果表明 $T^2$ 的分布来源于正态分布和 Wishart 分布的独立性。

• 多元正态随机向量 $N_p(0, \Sigma)$ 和样本协方差矩阵对应的 Wishart 随机矩阵 $W_{n-1}(\Sigma)$ 是**独立的**。

• 这一独立性是 $T^2$ 统计量分布性质的关键。
...
其中 $S^{-\tfrac12}$ 可以理解为任何满足 $S^{-\tfrac12}S^{-\tfrac12} = S^{-1}$ 的对称矩阵
...
这部分内容主要讨论了 **Hotelling’s $T^2$ 统计量** 的分布性质、与正态分布和 Wishart 分布的关系，以及其在变换（例如尺度变换）下的不变性。以下是逐条解释：
