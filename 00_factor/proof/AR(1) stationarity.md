...
- 这里证明的是弱平稳，不等同于严格平稳；白噪声只需二阶矩存在且与过去不相关即可支持上述协方差推导。
- $a_1=1$ 是随机游走，$|a_1|>1$ 是爆炸过程；$a_1=-1$ 时冲击不衰减并产生交替波动。
- 若存在确定性趋势或结构突变，应先处理确定性项，不能只用 $|a_1|<1$ 判断。

## 复现规范

报告 AR(1) 时同时记录截距、AR 系数、创新方差、样本区间和是否包含趋势项；实证中用单位根检验与残差诊断辅助，而不把有限样本估计值直接当作理论参数。
