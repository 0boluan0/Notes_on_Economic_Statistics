...
- 该推导依赖变量确实为同阶 $I(1)$，且 VAR 的滞后阶数、确定性项和秩设定正确；不同阶数变量不能直接套用普通 VECM 结论。
- $\beta$ 只在规范化和基变换意义下确定，协整空间确定后，具体向量表示仍需给出规范化约束。
- “存在协整”不自动意味着每个系数都有因果解释；需要进一步检查弱外生性、结构稳定性和残差诊断。

## 复现规范

复现 VECM 时记录单位根检验版本、VAR 滞后阶数、确定性项位置、秩检验方法与样本区间，并报告 $\hat\beta$、调整系数 $\hat\alpha$ 及残差稳定性。
